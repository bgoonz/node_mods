...
function readOTP(msg, otp, isRetry) {
  ...
  return read({ prompt: msg, default: otp || "" }).then((otp) =>
    readOTP(msg, otp, true)
  );
}
...
function readPassword(msg, password, isRetry) {
  ...
  return read({ prompt: msg, silent: true, default: password || "" }).then(
    (password) => readPassword(msg, password, true)
  );
}
...
function readUsername(msg, username, opts, isRetry) {
  ...
  return read({ prompt: msg, default: username || "" }).then((username) =>
    readUsername(msg, username, opts, true)
  );
}
...
function readEmail(msg, email, opts, isRetry) {
  ...
  return read({ prompt: msg, default: email || "" }).then((username) =>
    readEmail(msg, username, opts, true)
  );
}
