...
function getFundingInfo(idealTree, opts) {
  ...
  function hasDependencies(dependencies) {
    return (
      dependencies &&
      (Object.keys(dependencies).length || dependencies[_trailingDependencies])
    );
  }
    ...
    return directDepsWithFunding.reduce(
      (res, { dep: directDep, fundingItem }, i) => {
        ...
        if (hasDependencies(transitiveDependencies)) {
          fundingItem.dependencies = retrieveDependencies(
        ...
        if (fundingItem.funding && fundingItem.funding.length !== 0) {
          ...
          res[_trailingDependencies] = Object.assign(
            empty(),
