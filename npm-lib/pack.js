...
const prepublishWarning = require("./utils/warn-deprecated")(
...
function pack(args, silent, cb) {
  ...
  BB.all(args.map((arg) => pack_(arg, cwd)))
    .then((tarballs) => {
...
function packFromPackage(arg, target, filename) {
  ...
  return pacote.tarball
    .toFile(arg, target, opts)
    .then(() =>
      cacache.tmp.withTmp(npm.tmp, { tmpPrefix: "unpacking" }, (tmp) => {
        const tmpTarget = path.join(tmp, filename);
        return pacote
          .extract(arg, tmpTarget, opts)
  ...
  return readJson(path.join(dir, "package.json")).then((pkg) => {
    ...
    if (!pathIsInside(dir, npm.tmp)) {
      ...
      if (npm.config.get("ignore-prepublish")) {
        ...
        return lifecycle(pkg, "prepublish", dir)
          .then(() => {
...
function logContents(tarball) {
  ...
  log.notice(
    "",
    `${npm.config.get("unicode") ? "📦 " : "package:"} ${tarball.name}@${
      tarball.version
    }`
  );
  ...
  readJson(path.join(dir, "package.json"))
    .then((pkg) => {
      if (pkg.scripts && pkg.scripts.prepare) {
        log.verbose(
          "prepareGitDep",
          `${manifest._spec}: installing devDeps and running prepare script.`
        );
        ...
        return BB.fromNode((cb) => {
          ...
          child.on("exit", (code, signal) => {
            if (code > 0) {
              const err = new Error(
                `${signal}: npm exited with code ${code} while attempting to build ${manifest._requested}. Clone the repository manually and run 'npm install' in it for more information.`
              );
              err.code = code;
              err.signal = signal;
              cb(err);
          ...
          () => {
            if (outDataLen > 0)
              log.silly(
                "prepareGitDep",
                "1>",
                Buffer.concat(outData, outDataLen).toString()
              );
            if (errDataLen > 0)
              log.silly(
                "prepareGitDep",
                "2>",
                Buffer.concat(errData, errDataLen).toString()
              );
          },
          (err) => {
            if (outDataLen > 0)
