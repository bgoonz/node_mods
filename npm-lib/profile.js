...
function set(args) {
  ...
  return BB.try(() => {
    ...
    return pulseTillDone.withPromise(
      profile.get(conf).then((user) => {
        const newUser = {};
        writableProfileKeys.forEach((k) => {
          newUser[k] = user[k];
        });
        newUser[prop] = value;
        return otplease(conf, (conf) => profile.set(newUser, conf)).then(
          (result) => {
            if (conf.json) {
...
function enable2fa(args) {
  if (args.length > 1) {
    return Promise.reject(
      new Error("npm profile enable-2fa [auth-and-writes|auth-only]")
    );
  }
  ...
  return BB.try(() => {
    ...
    if (auth.basic) {
      ...
      return profile
        .createToken(auth.basic.password, false, [], conf)
        .then((result) => {
          if (!result.token)
            throw new Error(
              "Your registry " +
                conf.registry +
                "does not seem to support bearer tokens. Bearer tokens are required for two-factor authentication"
            );
          npm.config.setCredentialsByURI(conf.registry, {
            token: result.token,
          });
    ...
    .then((password) => {
      ...
      return pulseTillDone.withPromise(profile.get(conf)).then((info) => {
        ...
        if (info.tfa.pending) {
          ...
          return pulseTillDone.withPromise(
            profile.set({ tfa: { password, mode: "disable" } }, conf)
          );
        } else {
          if (conf.auth.otp) return;
          return readUserInfo
            .otp("Enter one-time password from your authenticator app: ")
            .then((otp) => {
              conf.auth.otp = otp;
            });
        }
    ...
    .then((challenge) => {
      ...
      if (
        typeof challenge.tfa !== "string" ||
        !/^otpauth:[/][/]/.test(challenge.tfa)
      ) {
        throw new Error(
          "Unknown error enabling two-factor authentication. Expected otpauth URL, got: " +
            inspect(challenge.tfa)
        );
      }
      const otpauth = url.parse(challenge.tfa);
      const opts = queryString.parse(otpauth.query);
      return qrcode(challenge.tfa)
        .then((code) => {
          output(
  ...
  return pulseTillDone.withPromise(profile.get(conf)).then((info) => {
    ...
    return readUserInfo.password().then((password) => {
      return BB.try(() => {
        if (conf.otp) return;
        return readUserInfo
          .otp("Enter one-time password from your authenticator: ")
          .then((otp) => {
        ...
        return pulseTillDone
          .withPromise(
            profile.set({ tfa: { password: password, mode: "disable" } }, conf)
          )
          .then(() => {
            if (conf.json) {
