...
try {
  ...
  log = {
    warn: function (m) {
      console.warn(
        m + " " + util.format.apply(util, [].slice.call(arguments, 1))
      );
    },
  };
}
...
nopt.invalidHandler = function (k, val, type) {
  ...
  switch (type) {
    case Umask:
      log.warn(
        "invalid config",
        "Must be umask, octal number in range 0000..0777"
      );
      break;
    case url:
...
Object.defineProperty(exports, "defaults", {
  get: function () {
    ...
    defaults = {
      ...
      usage: false,
      user:
        process.platform === "win32" || os.type() === "OS400" ? 0 : "nobody",
...
function getLocalAddresses() {
  ...
  return Object.keys(interfaces)
    .map((nic) => interfaces[nic].map(({ address }) => address))
