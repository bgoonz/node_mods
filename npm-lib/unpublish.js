  ...
  whoami([], true)
    .then((username) => {
      ...
      return libaccess.lsPackages(username, opts).then((access) => {
        ...
        if (!cliOpts.partialWord || !pkgs.length) {
          return pkgs;
        }
...
function unpublish(args, cb) {
  ...
  BB.try(() => {
    ...
    if (!spec || path.resolve(spec.name) === npm.localPrefix) {
      ...
      return readJson(cwdJson).then(
        (data) => {
          log.verbose("unpublish", data);
          return otplease(opts, (opts) => {
            return libunpub(
              npa.resolve(data.name, data.version),
              opts.concat(data.publishConfig)
            );
          });
        },
        (err) => {
    ...
    (ret) => {
      if (!opts.silent && opts.loglevel !== "silent") {
        output(
          `- ${spec.name}${spec.type === "version" ? `@${spec.rawSpec}` : ""}`
        );
      }
      cb(null, ret);
    },
