  ...
  move(
    ...
    .then(
      () => {
        log.notice(
          "",
          `${PKGLOCK} has been renamed to ${SHRINKWRAP}. ${SHRINKWRAP} will be used for future installations.`
        );
      ...
      (err) => {
        if (err.code !== "ENOENT") {
          ...
          return readPackageTree(npm.localPrefix)
  ...
  sortModules(tree.children).forEach(function (child) {
    ...
    if (child.requires.length) {
      ...
      sortModules(child.requires).forEach((required) => {
        var requested =
          getRequested(required, child) || required.package._requested || {};
        pkginfo.requires[moduleName(required)] = childRequested(
          top,
          required,
          requested
        );
...
function sortModules(modules) {
  ...
  return modules.sort(
    (a, b) =>
      sortedModuleNames.indexOf(moduleName(a)) -
      sortedModuleNames.indexOf(moduleName(b))
  );
}
...
function childVersion(top, child, req) {
  if (req.type === "directory" || req.type === "file") {
    return (
      "file:" +
      unixFormatPath(
        path.relative(top.path, child.package._resolved || req.fetchSpec)
      )
    );
...
function childRequested(top, child, requested) {
  if (requested.type === "directory" || requested.type === "file") {
    return (
      "file:" +
      unixFormatPath(
        path.relative(top.path, child.package._resolved || requested.fetchSpec)
      )
    );
    ...
    child.package._from ||
    (child.package._requested && child.package._requested.rawSpec)
  ) {
    return (
      child.package._from.replace(/^@?[^@]+@/, "") ||
      child.package._requested.rawSpec
    );
  } else {
    return child.package.version;
  }
  ...
  BB.join(
    ...
    (shrinkwrap, lockfile, pkg) => {
      ...
      if (swdata === info.raw) {
        ...
        log.verbose(
          "shrinkwrap",
          `skipping write for ${path.basename(
            info.path
        ...
        inferOwner(info.path).then((owner) => {
          writeFileAtomic(info.path, swdata, (err) => {
            ...
            if (!shrinkwrap && !lockfile) {
              log.notice(
                "",
                `created a lockfile as ${path.basename(
                  info.path
            ...
            if (
              selfOwner.uid === 0 &&
              (selfOwner.uid !== owner.uid || selfOwner.gid !== owner.gid)
            ) {
