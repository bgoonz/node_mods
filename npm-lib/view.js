  ...
  return packument(
    ...
    .then((data) => {
      ...
      if (silent) {
        ...
        throw error;
      } else if (!opts.json && args.length === 1 && args[0] === "") {
...
function prettyView(packument, manifest, opts) {
  ...
  return BB.try(() => {
    ...
    const unpackedSize =
      manifest.dist.unpackedSize && byteSize(manifest.dist.unpackedSize);
    ...
    const info = {
      ...
      modified: packument.time
        ...
        : undefined,
      maintainers: (packument.maintainers || []).map((u) =>
        unparsePerson({
      ...
      stars: color.yellow(
        "" + packument.users ? Object.keys(packument.users || {}).length : 0
      ),
      tags,
      ...
      unpackedSize:
        unpackedSize &&
        color.yellow(unpackedSize.value) + " " + unpackedSize.unit,
    };
    ...
    info.deprecated &&
      console.log(
        `\n${style.bright(color.red("DEPRECATED"))}${warningSign} - ${
          info.deprecated
        }`
      );
...
function search(data, fields, version, title) {
  ...
  if (Array.isArray(data)) {
    ...
    data.forEach(function (data, i) {
      ...
      const newt =
        title.substr(0, tl - fields.join(".").length - 1) +
        "[" +
        i +
        "]" +
        [""].concat(fields).join(".");
