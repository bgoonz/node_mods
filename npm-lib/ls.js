...
ls.usage = usage("ls", "npm ls [[<@scope>/]<pkg> ...]");
  ...
  if (!args) {
    ...
    args = args.map(function (a) {
      if (typeof a === "object" && a.package._requested.type === "alias") {
        ...
        var ver = p.rawSpec && (semver.validRange(p.rawSpec) || "");
        return [name, ver, a];
      }
    });
  }
...
function filterByEnv(data) {
  var dev =
    npm.config.get("dev") || /^dev(elopment)?$/.test(npm.config.get("only"));
  var production =
  ...
  Object.keys(data.dependencies).forEach(function (name) {
    ...
    if (
      (dev && inList(devKeys, name)) || // only --dev
...
function getLite(data, noname, depth) {
  ...
  if (data.extraneous) {
    ...
    lite.problems.push(
      "extraneous: " + packageId(data) + " " + (data.path || "")
    );
  }
  ...
  if (data.peerInvalid) {
    ...
    lite.problems = lite.problems || [];
    lite.problems.push(
      "peer dep not met: " + packageId(data) + " " + (data.path || "")
    );
  }
    ...
    if (depth - 1 <= npm.config.get("depth")) {
      ...
      return {
        ...
        nodes: Object.keys(data.dependencies || {})
          ...
          .map(function (d) {
            return makeArchy_(
              sortedObject(data.dependencies[d]),
              long,
              dir,
              depth + 1,
              data,
...
function makeParseable_(data, long, dir, depth, parent, d) {
    ...
    if (depth < npm.config.get("depth")) {
      data = npm.config.get("long")
        ? path.resolve(parent.path, "node_modules", d) +
          ...
          d +
          "@" +
          JSON.stringify(data.requiredBy) +
          ":INVALID:MISSING"
        : "";
    } else {
      data =
        path.resolve(dir || "", "node_modules", d || "") +
        (npm.config.get("long")
          ? ":" +
            d +
            "@" +
  ...
  return (
    ...
    (data.error && data.path !== path.resolve(npm.globalDir, "..")
      ? ":ERROR"
      : "") +
