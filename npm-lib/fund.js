...
function printHuman(fundingInfo, opts) {
  ...
  return archy({ label: `${name}${printableVersion}`, nodes: items }, "", {
...
function openFundingUrl(packageName, fundingSourceNumber, cb) {
  function getUrlAndOpen(packageMetadata) {
    ...
    const validSources = []
      .concat(retrieveFunding(funding))
      .filter(validFundingField);
    ...
    if (
      ...
      const { type, url } =
        validSources[fundingSourceNumber ? fundingSourceNumber - 1 : 0];
      ...
      const noFundingError = new Error(
        `No valid funding method available for: ${packageName}`
      );
  ...
  readPackageTree(dir, function (err, tree) {
    ...
    readShrinkwrap.andInflate(tree, function () {
      const fundingInfo = getFundingInfo(
        mutateIntoLogicalTree.asReadInstalled(computeMetadata(tree))
      );

      const print = opts.json ? printJSON : printHuman;

      output(print(fundingInfo, opts));
      cb(err, tree);
    });
  });
}
