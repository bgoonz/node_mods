...
function outdated(args, silent, cb) {
  ...
  readPackageTree(
    dir,
    andComputeMetadata(function (er, tree) {
      ...
      outdated_(args, "", tree, {}, 0, opts, function (er, list) {
        list = uniq(list || []).sort(function (aa, bb) {
          return (
            aa[0].path.localeCompare(bb[0].path) || aa[1].localeCompare(bb[1])
          );
        });
        if (er || silent || (list.length === 0 && !opts.json)) {
          return cb(er, list);
        }
        if (opts.json) {
...
function makePretty(p, opts) {
  ...
  var columns = [depname, has || "MISSING", want, latest, deppath || "global"];
  if (long) {
    columns[5] = type;
    columns[6] = homepage;
  }
...
function outdated_(args, path, tree, parentHas, depth, opts, cb) {
  ...
  var deps = tree.error
    ? tree.children
    : tree.children.filter((child) => !isExtraneous(child));
  ...
  var doUpdate =
    dev || (!prod && !Object.keys(parentHas).length && !opts.global);
  if (doUpdate) {
  ...
  asyncMap(
    ...
    function (dep, cb) {
      ...
      if (tree.package.dependencies && name in tree.package.dependencies) {
        ...
        name in tree.package.optionalDependencies
      ) {
        required = tree.package.optionalDependencies[name];
      } else if (
        tree.package.devDependencies &&
        name in tree.package.devDependencies
      ) {
