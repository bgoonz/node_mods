...
module.exports.login = function login(creds, registry, scope, cb) {
  ...
  if (!ssoType) {
    return cb(new Error("Missing option: sso-type"));
  }
  ...
  otplease(opts, (opts) =>
    profile.loginCouch(auth.username, auth.password, opts)
  )
    .then(({ token, sso }) => {
      if (!token) {
      ...
      return openUrl(sso, "to complete your login please visit")
        .then(() => {
...
function pollForSession(registry, token, opts) {
  ...
  return npmFetch
    .json("/-/whoami", opts.concat({ registry, forceAuth: { token } }))
    .then(
      ({ username }) => username,
      (err) => {
        if (err.code === "E401") {
