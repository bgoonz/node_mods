...
const openerPromise = (url) =>
  new Promise((resolve, reject) => {
    openUrl(url, "to complete your login please visit", (er) =>
...
const loginPrompter = (creds) => {
  const opts = { log: log };
  return read
    .username("Username:", creds.username, opts)
    .then((u) => {
      creds.username = u;
...
module.exports.login = (creds = {}, registry, scope, cb) => {
  ...
  login(opts)
    .then((newCreds) => cb(null, newCreds))
    .catch(cb);
};

function login(opts) {
  return profile
    .login(openerPromise, loginPrompter, opts)
    .catch((err) => {
