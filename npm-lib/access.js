...
access["ls-packages"] = access.lsPackages = ([owner], opts) => {
  return (owner ? BB.resolve(owner) : BB.fromNode((cb) => whoami([], true, cb)))
    .then((owner) => {
...
access["ls-collaborators"] = access.lsCollaborators = ([pkg, usr], opts) => {
  return getPackage(pkg, false)
    .then((pkgName) => libaccess.lsCollaborators(pkgName, usr, opts))
    .then((collabs) => {
      // TODO - print these out nicely (breaking change)
...
access["edit"] = () =>
  BB.reject(new Error("edit subcommand is not implemented yet"));
...
function getPackage(name, requireScope = true) {
  return BB.try(() => {
    if (name && name.trim()) {
      ...
      return readPackageJson(path.resolve(prefix, "package.json")).then(
        (data) => data.name,
        (err) => {
          if (err.code === "ENOENT") {
            throw new Error(
