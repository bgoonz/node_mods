...
function allPackageMetadata(opts) {
  ...
  opts = APMOpts(opts);
  const cacheBase = cacheFile(path.resolve(path.dirname(opts.cache)))(
    url.resolve(opts.registry, "/-/all")
  );
  const cachePath = path.join(cacheBase, ".cache.json");
  createEntryStream(cachePath, staleness, opts)
    .then(({ entryStream, latest, newEntries }) => {
      log.silly("all-package-metadata", "entry stream created");
      if (entryStream && newEntries) {
        return createCacheWriteStream(cachePath, latest, opts).then(
          (writer) => {
...
function createEntryStream(cachePath, staleness, opts) {
  return createCacheEntryStream(cachePath, opts)
    .catch((err) => {
      log.warn("", "Failed to read search cache. Rebuilding");
      log.silly("all-package-metadata", "cache read error: ", err);
      return {};
    })
    .then(({ updateStream: cacheStream, updatedLatest: cacheLatest }) => {
      cacheLatest = cacheLatest || 0;
      return createEntryUpdateStream(staleness, cacheLatest, opts)
        .catch((err) => {
...
function createEntryUpdateStream(staleness, latest, opts) {
  ...
  if (latest && Date.now() - latest < staleness * 1000) {
    ...
    log.verbose(
      ...
      "Local data up to date, skipping update"
    );
    return BB.resolve({});
  } else if (latest === 0) {
    log.warn(
    ...
    log.verbose(
      "all-package-metadata",
      "No cached data: requesting full metadata db"
    );
  } else {
    log.verbose(
      "all-package-metadata",
      "Cached data present with timestamp:",
      latest,
      "requesting partial index update"
    );
...
function createCacheWriteStream(cacheFile, latest, opts) {
  return _ensureCacheDirExists(cacheFile, opts).then(({ uid, gid }) => {
    ...
    linkStreams(inputStream, outStream, () => {
      errEmitted = true;
    });
...
function _ensureCacheDirExists(cacheFile, opts) {
  ...
  log.silly(
    "all-package-metadata",
    "making sure cache dir exists at",
    cacheBase
  );
