...
function list(args) {
  ...
  return pulseTillDone.withPromise(profile.listTokens(conf)).then((tokens) => {
    ...
    const idWidth = tokens.reduce(
      (acc, token) => Math.max(acc, token.id.length),
      0
    );
...
function rm(args) {
  ...
  return pulseTillDone
    .withPromise(
      profile.listTokens(conf).then((tokens) => {
        args.forEach((id) => {
          ...
          if (matches.length === 1) {
            ...
            throw new Error(
              `Token ID "${id}" was ambiguous, a new token may have been created since you last ran \`npm-profile token list\`.`
            );
          } else {
            const tokenMatches = tokens.filter(
              (token) => id.indexOf(token.token) === 0
            );
            if (tokenMatches === 0) {
