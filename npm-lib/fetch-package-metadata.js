...
function fetchPackageMetadata(spec, where, opts, done) {
  ...
  if (
    !isWindows &&
    dep.type === "directory" &&
    /^[a-zA-Z]:/.test(dep.fetchSpec)
  ) {
    var err = new Error(
      `Can't install from windows path on a non-windows system: ${dep.fetchSpec.replace(
        /[/]/g,
        "\\"
      )}`
    );
  ...
  pacote
    ...
    .then(
      ...
      (err) => {
        ...
        if (err.code === "ENOTDIR") {
          var enolocal = new Error(
            `Could not install "${path.relative(
              process.cwd(),
              dep.fetchSpec
            )}" as it is not a directory and is not a file with a name ending in .tgz, .tar.gz or .tar`
          );
          enolocal.code = "ENOLOCAL";
          if (err.stack) enolocal.stack = err.stack;
          return logAndFinish(enolocal);
        } else if (err.code === "ENOPACKAGEJSON") {
          var enopackage = new Error(
            `Could not install from "${path.relative(
              process.cwd(),
              dep.fetchSpec
...
function addBundled(pkg, next) {
  ...
  if (!pkg.bundleDependencies && pkg._requested.type !== "directory")
  ...
  pacote
    .extract(
      pkg._resolved || pkg._requested || npa.resolve(pkg.name, pkg.version),
      target,
      opts
    )
    .then(() => {
