  ...
  npm.config.save(which, function (er) {
    ...
    fs.readFile(f, "utf8", function (er, data) {
      ...
      data = [
        ";;;;",
        "; npm " +
          (npm.config.get("global") ? "globalconfig" : "userconfig") +
          " file",
        ...
        .concat([";;;;", "; all options with default values", ";;;;"])
        .concat(
          Object.keys(npmconf.defaults).reduce(function (arr, key) {
            var obj = {};
...
function list(cb) {
  ...
  msg += listFromSource(
    ...
    npm.config.sources.global.data,
    long
  );
