...
function Installer(where, dryrun, args, opts) {
  ...
  this.dev =
    opts.dev != null ? opts.dev : dev || (!onlyProd && !prod) || onlyDev;
  this.prod = opts.prod != null ? opts.prod : !onlyDev;

  this.packageLockOnly =
    opts.packageLockOnly != null
      ? opts.packageLockOnly
      : npm.config.get("package-lock-only");
  this.rollback =
    opts.rollback != null ? opts.rollback : npm.config.get("rollback");
  this.link = opts.link != null ? opts.link : npm.config.get("link");
  this.saveOnlyLock = opts.saveOnlyLock;
  this.global =
    opts.global != null
      ? opts.global
      : this.where === path.resolve(npm.globalDir, "..");
  ...
  if (this.currentTree.error) {
    for (let child of this.currentTree.children) {
      if (!child.fakeChild && isExtraneous(child)) {
        this.currentTree.package.dependencies[moduleName(child)] =
          computeVersionSpec(this.currentTree, child);
      }
    }
  }
...
Installer.prototype.pruneIdealTree = function (cb) {
  ...
  if (
    !this.idealTree.hasRequiresFromLock &&
    this.idealTree.children.some((n) => n.fakeChild)
  )
    return cb();
  const toPrune = this.idealTree.children
    .filter(
      (child) => isExtraneous(child) && (this.autoPrune || child.removing)
    )
...
Installer.prototype.loadAllDepsIntoIdealTree = function (cb) {
  ...
  steps.push([
    loadExtraneous.andResolveDeps,
    this.idealTree,
    cg.newGroup("loadExtraneous"),
  ]);
  chain(steps, cb);
};
...
function isLinkable(pkg, cb) {
  var globalPackage = path.resolve(
    npm.globalPrefix,
    "lib",
    "node_modules",
    moduleName(pkg)
...
Installer.prototype.executeActions = function (cb) {
  ...
  steps.push(
    [
      ...
      staging,
      todo,
      trackLifecycle.newGroup("global-install"),
    ],
    ...
    [
      ...
      staging,
      todo,
      cg.newGroup("refresh-package-json"),
    ],
    [
      doParallelActions,
      "preinstall",
      staging,
      todo,
      trackLifecycle.newGroup("preinstall"),
    ],
    ...
    [
      ...
      staging,
      todo,
      trackLifecycle.newGroup("global-link"),
    ],
    [
      doParallelActions,
      "update-linked",
      staging,
      todo,
      trackLifecycle.newGroup("update-linked"),
    ],
    [
      doSerialActions,
      "install",
      staging,
      todo,
      trackLifecycle.newGroup("install"),
    ],
    [
      doSerialActions,
      "postinstall",
      staging,
      todo,
      trackLifecycle.newGroup("postinstall"),
    ]
  );
...
Installer.prototype.rollbackFailedOptional = function (
  staging,
  actionsToRun,
  cb
) {
  ...
  Bluebird.map(failed, (pkg) => {
    return Bluebird.map(pkg.rollback, (rollback) =>
      rollback(top, staging, pkg)
    );
  }).asCallback(cb);
};

Installer.prototype.commit = function (staging, actionsToRun, cb) {
  var toCommit = actionsToRun
    .map(function (action) {
      return action[1];
    })
    .filter(function (pkg) {
...
Installer.prototype.runPreinstallTopLevelLifecycles = function (cb) {
  ...
  readPackageJson(
    path.join(this.where, "package.json"),
    log,
    false,
    (err, data) => {
      if (err) return cb();
...
Installer.prototype.runPostinstallTopLevelLifecycles = function (cb) {
  ...
  if (this.dev) {
    steps.push([
      ...
      this.idealTree.path,
      this.idealTree,
      trackLifecycle.newGroup("prepare"),
    ]);
  }
  chain(steps, cb);
};
...
Installer.prototype.startAudit = function (cb) {
  ...
  this.auditSubmission = Bluebird.try(() => {
    return audit.generateFromInstall(
      this.idealTree,
      this.differences,
      this.args,
      this.remove
      ...
      readPackageTree(
        ...
        iferr(cb, function (currentTree) {
          ...
          if (!self.noPackageJsonOk && !currentTree.package) {
            ...
            var er = new Error(
              "ENOENT, open '" + path.join(self.where, "package.json") + "'"
            );
...
Installer.prototype.printWarnings = function (cb) {
  ...
  if (warned && log.levels[npm.config.get("loglevel")] <= log.levels.warn)
    console.error();
  cb();
};
...
Installer.prototype.printInstalled = function (cb) {
  ...
  return Bluebird.try(() => {
    ...
    return Bluebird.resolve(this.auditSubmission)
      .timeout(10000)
      .catch(() => null);
  })
    .then((auditResult) => {
      if (auditResult && !auditResult.metadata) {
        log.warn(
          "audit",
          "Audit result from registry missing metadata. This is probably an issue with the registry."
        );
      }
...
Installer.prototype.printInstalledForJSON = function (diffs, auditResult) {
  ...
  function recordAction(action) {
    ...
    const isAlias =
      ...
      child.package._requested.type === "alias";
    const name = isAlias
    ...
    var result = {
      ...
      version:
        child.package &&
        `${isAlias ? `npm:${child.package.name}@` : ""}${
    ...
    if (mutation === "move") {
      ...
      result.previousVersion =
        child.oldPkg.package && child.oldPkg.package.version;
    }
    return result;
  }
};
...
Installer.prototype.printInstalledForParseable = function (diffs) {
  ...
  diffs.forEach(function (action) {
    ...
    const isAlias =
      child.package._requested && child.package._requested.type === "alias";
    const version =
      child.package && isAlias
...
Installer.prototype.debugLogicalTree = function (name, treeName, cb) {
  validate("SSF", arguments);
  this[treeName] &&
    log.silly(name, this.archyDebugLogicalTree(this[treeName]).trim());
  cb();
};
