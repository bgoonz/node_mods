  ...
  return Bluebird.all([
    ...
    .spread((shrinkwrap, lockfile, pkgJson) => {
      ...
      if (!sw) {
        ...
        log.warn(
          "audit",
          "Both npm-shrinkwrap.json and package-lock.json exist, using npm-shrinkwrap.json."
        );
      }
      ...
      return lockVerify(npm.prefix).then((result) => {
        ...
        const lockFile = shrinkwrap
          ? "npm-shrinkwrap.json"
          : "package-lock.json";
        const err = new Error(
          `Errors were found in your ${lockFile}, run  npm install  to fix them.\n    ` +
            result.errors.join("\n    ")
        );
    ...
    .then((auditResult) => {
      if (args[0] === "fix") {
        const actions = (auditResult.actions || []).reduce(
          (acc, action) => {
            ...
            if (action.isMajor) {
              ...
              action.resolves.forEach(({ id, path }) =>
                acc.majorFixes.add(`${id}::${path}`)
              );
            } else if (action.action === "install") {
              acc.install.add(`${action.module}@${action.target}`);
              action.resolves.forEach(({ id, path }) =>
                acc.installFixes.add(`${id}::${path}`)
              );
              ...
              action.resolves.forEach((vuln) => {
                ...
                const newPath = modPath
                  .slice(0, modPath.indexOf(name))
                  .concat(`${name}@${version}`);
                if (newPath.length === 1) {
        ...
        return Bluebird.try(() => {
          ...
          const installCount =
            actions.install.size +
            (installMajor ? actions.major.size : 0) +
            actions.update.size;
          const vulnFixCount = new Set([
            ...actions.installFixes,
            ...actions.updateFixes,
            ...(installMajor ? actions.majorFixes : []),
          ]).size;
          const metavuln = auditResult.metadata.vulnerabilities;
          const total = Object.keys(metavuln).reduce(
            (acc, key) => acc + metavuln[key],
            0
          );
          if (installCount) {
            log.verbose("audit", "installing", [
              ...actions.install,
              ...(installMajor ? actions.major : []),
              ...actions.update,
            ]);
          }
          return Bluebird.fromNode((cb) => {
            ...
            if (!opts.json && !opts.parseable) {
              output(
                `fixed ${vulnFixCount} of ${total} vulnerabilit${
                  total === 1 ? "y" : "ies"
                } in ${numScanned} scanned package${
                  numScanned === 1 ? "" : "s"
                }`
              );
              if (actions.review.size) {
                output(
                  `  ${actions.review.size} vulnerabilit${
                    actions.review.size === 1 ? "y" : "ies"
                  } required manual review and could not be updated`
                );
              }
              if (actions.major.size) {
                output(
                  `  ${actions.major.size} package update${
                    actions.major.size === 1 ? "" : "s"
                  } for ${actions.majorFixes.size} vulnerabilit${
                    actions.majorFixes.size === 1 ? "y" : "ies"
                  } involved breaking changes`
                );
                if (installMajor) {
        ...
        const vulns = levels.reduce((count, level, i) => {
          return i < minLevel
            ? count
            : count + (auditResult.metadata.vulnerabilities[level] || 0);
        }, 0);
