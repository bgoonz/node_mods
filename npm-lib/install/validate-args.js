...
function hasMinimumFields(pkg, cb) {
  if (pkg.name === "" || pkg.name == null) {
    return cb(
      new Error(`Can't install ${pkg._resolved}: Missing package name`)
    );
  } else if (pkg.version === "" || pkg.version == null) {
    return cb(
      new Error(`Can't install ${pkg._resolved}: Missing package version`)
    );
  } else {
    return cb();
  }
...
var isInstallable = (module.exports.isInstallable = function (
  idealTree,
  pkg,
  next
) {
  ...
  checkEngine(
    ...
    force,
    strict,
    iferr(next, thenWarnEngineIssues)
  );
...
function checkSelf(idealTree, pkg, force, next) {
  ...
  if (force) {
    var warn = new Error(
      "Wouldn't install " +
        pkg.name +
        " as a dependency of itself, but being forced"
    );
    warn.code = "ENOSELF";
