...
function submitForInstallReport(auditData) {
  ...
  const scopedRegistries = [...opts.keys()]
    .filter((k) => /:registry$/.test(k))
    .map((k) => opts[k]);
...
function scrubDeps(deps) {
  ...
  Object.keys(deps).forEach((name) => {
    if (!shouldScrubName(name) && !shouldScrubSpec(name, deps[name].version))
      return;
...
function scrub(value, rid) {
  return ssri
    .fromData((rid || runId) + " " + value, { algorithms: ["sha256"] })
    .hexDigest();
}
...
function generateFromInstall(tree, diffs, install, remove) {
  ...
  tree.requires.forEach((pkg) => {
    requires[pkg.package.name] =
      tree.package.dependencies[pkg.package.name] ||
      tree.package.devDependencies[pkg.package.name] ||
      pkg.package.version;
  });
  ...
  diffs.forEach((action) => {
    ...
    if (mutation !== "add" && mutation !== "update" && mutation !== "remove")
      return;
