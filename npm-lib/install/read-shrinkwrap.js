...
const PKGLOCK_VERSION = require("../npm.js.js").lockfileVersion;
  ...
  BB.join(
    ...
    (shrinkwrap, lockfile) => {
      if (shrinkwrap && lockfile) {
        log.warn(
          "read-shrinkwrap",
          "Ignoring package-lock.json because there is already an npm-shrinkwrap.json. Please use only one of the two."
        );
      }
      ...
      if (parsed && parsed.lockfileVersion !== PKGLOCK_VERSION) {
        log.warn(
          "read-shrinkwrap",
          `This version of npm is compatible with lockfileVersion@${PKGLOCK_VERSION}, but ${name} was generated for lockfileVersion@${
            parsed.lockfileVersion || 0
...
function parsePkgLock(str, filename) {
  ...
  try {
    ...
    if (isDiff(str)) {
      log.warn(
        "conflict",
        `A git conflict was detected in ${filename}. Attempting to auto-resolve.`
      );
      log.warn(
        "conflict",
        "To make this happen automatically on git rebase/merge, consider using the npm-merge-driver:"
      );
      ...
      try {
        ...
        log.error(
          "conflict",
          `Automatic conflict resolution failed. Please manually resolve conflicts in ${filename} and try again.`
        );
