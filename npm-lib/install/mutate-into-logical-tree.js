...
function translateTree_(tree, seen) {
  ...
  tree.children.forEach(function (child) {
    const dep = (pkg.dependencies[moduleName(child)] = translateTree_(
      child,
      seen
    ));
    if (child.fakeChild) {
  ...
  pkg.extraneous =
    !tree.isTop &&
    (!tree.parent.isTop || !tree.parent.error) &&
    !npm.config.get("global") &&
    isExtraneous(tree);
  if (tree.target && tree.parent && !tree.parent.target)
    pkg.link = tree.realpath;
  return pkg;
}
