...
function saveShrinkwrap(tree, next) {
  ...
  if (
    ...
    (!npm.config.get("shrinkwrap") || !npm.config.get("package-lock"))
  ) {
    return next();
  }
...
function savePackageJson(tree, next) {
  ...
  fs.readFile(
    ...
    iferr(next, function (packagejson) {
      ...
      if (saveBundle) {
        var bundle =
          tree.package.bundleDependencies || tree.package.bundledDependencies;
      ...
      toSave.forEach(function (pkg) {
        ...
        if (movedFrom.length) {
          log.notice(
            "save",
            `${pkg.name} is being moved from ${movedFrom.join(" and ")} to ${
              pkg.save
            }`
          );
        }
