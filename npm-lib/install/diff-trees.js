...
function pkgRequested(pkg) {
  return (
    pkg._requested ||
    (pkg._resolved && npa(pkg._resolved)) ||
    (pkg._from && npa(pkg._from))
  );
}
...
function pkgAreEquiv(aa, bb) {
  ...
  if (fromGit(aaReq) && fromGit(bbReq)) {
    ...
    return (
      aa.package._resolved &&
      bb.package._resolved &&
...
var diffTrees = (module.exports._diffTrees = function (oldTree, newTree) {
  ...
  Object.keys(flatOldTree).forEach(function (flatname) {
    ...
    if (
      pkg.isInLink &&
      /^[.][.][/\\]/.test(path.relative(newTree.realpath, pkg.realpath))
    )
      return;
  ...
  Object.keys(flatNewTree).forEach(function (flatname) {
    ...
    if (oldPkg) {
      ...
      var removing = (toRemoveByName[name] || []).filter((rm) =>
        pkgAreEquiv(rm.pkg, pkg)
      );
      var bundlesOrFromBundle =
        pkg.fromBundle || pkg.package.bundleDependencies;
...
function filterActions(differences) {
  ...
  return differences.filter((diff) => {
    ...
    if (includeProd && !pkgIsOnlyDev && (includeOpt || !pkgIsOnlyOpt))
      return true;
    return false;
  });
}
