...
const checkPlatform = Bluebird.promisify(
  require("npm-install-checks").checkPlatform
);
...
module.exports = function (staging, pkg, log) {
  ...
  return readJson(path.join(pkg.path, "package.json"), false)
    .then((metadata) => {
      Object.keys(pkg.package).forEach(function (key) {
        if (
          key !== "version" &&
          key !== "dependencies" &&
          !isEmpty(pkg.package[key])
        ) {
