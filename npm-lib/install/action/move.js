...
function moveModuleOnly(from, to, log, done) {
  ...
  move(toModules, tempToModules).then(
    removeDestination(done),
    removeDestination(done)
  );
  ...
  function removeDestination(next) {
    return function (er) {
      ...
      if (er) {
        ...
        rimraf(
          to,
          iferr(next, makeDestination(iferr(next, moveToModulesBack(next))))
        );
      }
    };
  }
  ...
  function moveToModulesBack(next) {
    return function () {
      log.silly(
        "move",
        "move existing destination node_modules back",
        toModules
      );
  ...
  function moveNodeModules(next) {
    return function () {
      ...
      move(fromModules, tempFromModules).then(
        doMove(moveNodeModulesBack(next)),
        doMove(next)
      );
    };
  }
