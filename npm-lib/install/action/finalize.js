...
module.exports = function (staging, pkg, log) {
  ...
  const delpath = path.join(
    path.dirname(pkg.realpath),
    "." + path.basename(pkg.realpath) + ".DELETE"
  );
  ...
  if (requested.type === "directory") {
    ...
    return makeParentPath(pkg.path)
      ...
      .catch((ex) => {
        return rimraf(pkg.path).then(() =>
          symlink(relative, pkg.path, "junction")
        );
      });
  } else {
    return makeParentPath(pkg.realpath)
      ...
      .catch((err) => {
        if (movedDestAway) {
          return rimraf(pkg.realpath)
            .then(moveOldDestinationBack)
            .then(() => {
              throw err;
            });
        } else {
  ...
  function restoreOldNodeModules() {
    ...
    return readdir(path.join(delpath, "node_modules"))
      .catch(() => [])
      .then((modules) => {
        if (!modules.length) return;
        return mkdirp(path.join(pkg.realpath, "node_modules")).then(() =>
          Bluebird.map(modules, (file) => {
...
module.exports.rollback = function (top, staging, pkg) {
  return Bluebird.try(() => {
    ...
    return gentlyRm(pkg.path, false, top).catch((err) => {
      log.warn(
        "rollback",
        `Rolling back ${packageId(pkg)} failed (this is probably harmless): ${
