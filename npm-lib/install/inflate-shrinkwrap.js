...
module.exports = function (tree, sw, opts, finishInflating) {
  if (!fetchPackageMetadata) {
    fetchPackageMetadata = BB.promisify(
      require("../fetch-package-metadata.js")
    );
...
function tarballToVersion(name, tb) {
  ...
  if (name) {
    ...
    matchRegTarball = new RegExp(
      `^${registry}${nameMatch}/-/${nameMatch}-(.*)[.]tgz$`
    );
  } else {
...
function relativizeLink(name, spec, topPath, requested) {
  ...
  const relativized = path.relative(
    requestedPath,
    path.resolve(topPath, spec.slice(5))
  );
  return "file:" + relativized;
}

function inflatableChild(
  onDiskChild,
  name,
  topPath,
  tree,
  ...
  if (
    hasModernMeta(onDiskChild) &&
    childIsEquivalent(sw, requested, onDiskChild)
  ) {
    ...
    onDiskChild.fromBundle =
      sw.bundled || onDiskChild.package._inBundle
        ? tree.fromBundle || tree
        : null;
    ...
    (sw.version &&
      (sw.integrity || !usesIntegrity) &&
      (requested.type !== "directory" || modernLink)) ||
    sw.bundled
  ) {
...
function makeFakeChild(name, topPath, tree, sw, requested) {
  ...
  if (!sw.bundled) {
    const bundleDependencies = Object.keys(sw.dependencies || {}).filter(
      (d) => sw.dependencies[d].bundled
    );
...
function fetchChild(topPath, tree, sw, requested) {
  return fetchPackageMetadata(requested, topPath)
    .then((pkg) => {
...
function childIsEquivalent(sw, requested, child) {
  ...
  if (
    ...
    return true;
  if (child.isLink && requested.type === "directory")
  ...
  if (!isRegistry(requested) && child.package._resolved)
    return sw.version === child.package._resolved;
