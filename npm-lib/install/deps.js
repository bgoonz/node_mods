...
function doesChildVersionMatch(child, requested, requestor) {
  ...
  if (requested.type === "git" && child.fromShrinkwrap) {
    const fromSw = child.package._from
      ? npa(child.package._from)
  ...
  if (!registryTypes[requested.type]) {
    ...
    if (childReq) {
      ...
      if (childReq.type === requested.type) {
        ...
        if (
          childReq.fetchSpec === requested.fetchSpec &&
          requested.type !== "git"
        )
          return true;
      }
...
function computeMetadata(tree, seen) {
  ...
  tree.children
    ...
    .forEach((child) => computeMetadata(child, seen));
...
function isDep(tree, child) {
  ...
  try {
    ...
    return {
      isDep:
        doesChildVersionMatch(child, devSpec, tree) || child.fromShrinkwrap,
      isProdDep: false,
      isDevDep: devSpec,
    };
  } catch (err) {
...
function matchingDep(tree, name) {
  ...
  if (tree.package.dependencies && tree.package.dependencies[name])
    return tree.package.dependencies[name];
  if (tree.package.devDependencies && tree.package.devDependencies[name])
    return tree.package.devDependencies[name];
}
      ...
      if (
        ...
        return fs.stat(path.join(arg, "package.json"), (err) => {
          if (err) {
            ...
            if (version) {
              try {
                return fetchPackageMetadata(
                  npa.resolve(spec.name, version),
                  where,
                  done
                );
              } catch (e) {
...
exports.loadRequestedDeps = function (
  args,
  tree,
  saveToDependencies,
  log,
  ...
  asyncMap(
    ...
    function (pkg, done) {
      ...
      resolveWithNewModule(
        pkg,
        tree,
        log.newGroup("loadRequestedDeps"),
        iferr(depLoaded, function (child, tracker) {
          ...
          if (child.save) {
            ...
            if (child.save === "optionalDependencies")
...
function computeVersionSpec(tree, child) {
  ...
  if (child.isLink) {
    ...
    childReq &&
    (isNotEmpty(childReq.saveSpec) ||
      (isNotEmpty(childReq.rawSpec) && isNotEmpty(childReq.fetchSpec)))
  ) {
  ...
  if (isRegistry(requested)) {
    ...
    return (
      "file:" +
      unixFormatPath(path.relative(getTop(tree).path, requested.fetchSpec))
    );
  } else {
...
exports.removeDeps = function (args, tree, saveToDependencies, next) {
  ...
  for (let pkg of args) {
    ...
    if (tree.package[saveType][pkgName]) {
      ...
      if (
        saveType === "optionalDependencies" &&
        tree.package.dependencies[pkgName]
      ) {
    ...
    for (let parent of pkgToRemove.requiredBy) {
      parent.requires = parent.requires.filter(
        (child) => child !== pkgToRemove
      );
    }
    pkgToRemove.requiredBy = pkgToRemove.requiredBy.filter(
      (parent) => parent !== tree
    );
...
function isDepOptional(tree, name, pkg) {
  ...
  if (devDeps && devDeps[name] != null) {
    const includeDev =
      npm.config.get("dev") ||
      (!/^prod(uction)?$/.test(npm.config.get("only")) &&
...
function prefetchDeps(tree, deps, log, next) {
  ...
  function resolveBranchDeps(pkg, deps) {
    return BB.resolve(null).then(() => {
      var allDependencies = Object.keys(deps)
        .map((dep) => {
        ...
        return fpm(dep, "", { tracker: log.newItem("fetchMetadata") }).then(
          (pkg) => {
            return (
              pkg &&
              pkg.dependencies &&
              resolveBranchDeps(pkg, pkg.dependencies)
            );
          },
          () => null
        );
...
function loadDeps(tree, log, next) {
  ...
  if (tree.loaded || (tree.parent && tree.parent.failed) || tree.removed)
  ...
  asyncMap(
    Object.keys(tree.package.dependencies),
    function (dep, done) {
      ...
      addDependency(
        ...
        version,
        tree,
        log.newGroup("loadDep:" + dep),
        andHandleOptionalErrors(log, tree, dep, done)
      );
    },
    andForEachChild(loadDeps, andFinishTracker(log, next))
  );
...
exports.loadDevDeps = function (tree, log, next) {
  ...
  asyncMap(
    Object.keys(tree.package.devDependencies),
    function (dep, done) {
      ...
      addDependency(
        dep,
        tree.package.devDependencies[dep],
        tree,
        logGroup,
        andHandleOptionalErrors(log, tree, dep, done)
      );
...
function reportBundleOverride(child, log) {
  ...
  if (log)
    log.verbose(
      "bundle",
      `${code}: Replacing ${bundlerId}'s bundled version of ${moduleName(
        child
...
function resolveWithExistingModule(child, tree) {
  ...
  if (tree.parent && child.parent !== tree)
    updatePhantomChildren(tree.parent, child);
}

var updatePhantomChildren = (exports.updatePhantomChildren = function (
  current,
  child
) {
  validate("OO", arguments);
  ...
  return isInstallable(tree, pkg, (err) => {
    ...
    addBundled(pkg, (bundleErr) => {
      ...
      var child = createChild({
        ...
        parent: parent,
        path: path.join(
          parent.isLink ? parent.realpath : parent.path,
          "node_modules",
          name
        ),
        realpath: isLink
          ? pkg._requested.fetchSpec
          : path.join(parent.realpath, "node_modules", name),
...
var validatePeerDeps = (exports.validatePeerDeps = function (tree, onInvalid) {
  ...
  Object.keys(tree.package.peerDependencies).forEach(function (pkgname) {
    ...
    if (!match && !isOptionalPeerDep(tree, pkgname))
      onInvalid(tree, pkgname, version);
  });
});
...
var findRequirement = (exports.findRequirement = function (
  tree,
  name,
  requested,
  requestor
  ...
  if (
    !preserveSymlinks() &&
    /^[.][.][\\/]/.test(path.relative(tree.parent.realpath, tree.realpath))
  )
    return null;
...
var earliestInstallable = (exports.earliestInstallable = function (
  requiredBy,
  tree,
  pkg,
  log
  ...
  function undeletedModuleMatches(child) {
    return (
      !child.removed &&
      moduleName(child) ===
        ((pkg._requested && pkg._requested.name) || pkg.name)
    );
  }
