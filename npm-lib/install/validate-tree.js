...
function checkErrors(mod, idealTree, next) {
  if (
    mod.error &&
    (mod.parent || path.resolve(npm.globalDir, "..") !== mod.path)
  )
    idealTree.warnings.push(mod.error);
  next();
}
  ...
  for (var pkg in devDeps) {
    if (pkg in deps) {
      var warnObj = new Error(
        "The package " +
          pkg +
          " is included as both a dev and production dependency."
      );
