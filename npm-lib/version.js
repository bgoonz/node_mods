...
function version(args, silent, cb_) {
  ...
  readPackage(function (er, data, indent, newline) {
    ...
    if (args[0] === "from-git") {
      ...
      if (!newVersion)
        newVersion = semver.inc(data.version, args[0], npm.config.get("preid"));
...
function parseLastGitTag(cb) {
  ...
  git.whichAndExec(["describe", "--abbrev=0"], options, function (er, stdout) {
    if (er) {
      if (er.message.indexOf("No names found") !== -1)
        return cb(new Error("No tags found"));
      return cb(er);
    }
...
function persistVersion(newVersion, silent, data, localData, cb_) {
  ...
  if (!npm.config.get("allow-same-version") && data.version === newVersion) {
    return cb_(
      new Error("Version not changed, might want --allow-same-version")
    );
  }
...
function callGitStatus(cb) {
  git.whichAndExec(["status", "--porcelain"], { env: process.env }, cb);
}
...
function checkGit(localData, cb) {
  statGitFolder(function (er) {
    ...
    if (!doGit) {
      if (er && npm.config.get("git-tag-version"))
        log.verbose("version", "error checking for .git", er);
...
function _commit(version, localData, cb) {
  ...
  stagePackageFiles(localData, options)
    .then(() => {
...
function write(data, file, indent, newline, cb) {
  ...
  assert(
    typeof file === "string",
    "must pass filename to write to version write"
  );
