...
function testbinary(gyp, argv, callback) {
  ...
  if (nw) {
    ...
    log.info(
      "validate",
      "Running test command: '" + shell_cmd + " " + args.join(" ") + "'"
    );
    cp.execFile(shell_cmd, args, options, (err, stdout, stderr) => {
      ...
      if (err) {
        ...
        if (
          /^\s*Xlib:\s*extension\s*"RANDR"\s*missing\s*on\s*display\s*":\d+\.\d+"\.\s*$/.test(
            stderrLog
          )
        ) {
