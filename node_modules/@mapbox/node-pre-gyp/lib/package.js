...
exports.usage =
  "Packs binary (and enclosing directory) into locally staged tarball";
...
function _package(gyp, argv, callback) {
  ...
  existsAsync(binary_module, (found) => {
    if (!found) {
      return callback(
        new Error(
          "Cannot package because " +
            binary_module +
            " missing: run `node-pre-gyp rebuild` first"
        )
      );
    }
    ...
    makeDir(path.dirname(tarball))
      .then(() => {
        ...
        tar.create(
          ...
          (err2) => {
            if (err2)
              console.error("[" + package_json.name + "] " + err2.message);
