...
module.exports.validate_package_json = function (package_json, opts) {
  ...
  const napi_build_versions = module.exports.get_napi_build_versions(
    package_json,
    opts,
    true
  );
  const napi_build_versions_raw =
  ...
  if (napi_build_versions) {
    napi_build_versions.forEach((napi_build_version) => {
      if (
        !(
          parseInt(napi_build_version, 10) === napi_build_version &&
  ...
  if (
    napi_build_versions &&
    (!module_path_ok || (!remote_path_ok && !package_name_ok))
  ) {
    throw new Error(
  ...
  if (
    ...
    throw new Error(
      "The Node-API version of this Node instance is " +
        module.exports.get_napi_version(opts ? opts.target : undefined) +
        ". " +
...
function pathOK(path) {
  return (
    path &&
    (path.indexOf("{napi_build_version}") !== -1 ||
      path.indexOf("{node_napi_label}") !== -1)
  );
}
...
module.exports.expand_commands = function (package_json, opts, commands) {
  ...
  commands.forEach((command) => {
    if (napi_build_versions && command.name === "install") {
      const napi_build_version = module.exports.get_best_napi_build_version(
        package_json,
        opts
      );
      ...
      napi_build_versions &&
      napi_multiple_commands.indexOf(command.name) !== -1
    ) {
      napi_build_versions.forEach((napi_build_version) => {
...
module.exports.get_napi_build_versions = function (
  package_json,
  opts,
  warnings
) {
  ...
  const supported_napi_version = module.exports.get_napi_version(
  ...
  if (package_json.binary && package_json.binary.napi_versions) {
    package_json.binary.napi_versions.forEach((napi_version) => {
      ...
      if (
        !duplicated &&
        supported_napi_version &&
        napi_version <= supported_napi_version
      ) {
...
module.exports.get_napi_build_version_from_command_args = function (
  command_args
) {
...
module.exports.get_best_napi_build_version = function (package_json, opts) {
  ...
  if (napi_build_versions) {
    const our_napi_version = module.exports.get_napi_version(
      opts ? opts.target : undefined
    );
    napi_build_versions.forEach((napi_build_version) => {
      if (
        napi_build_version > best_napi_build_version &&
