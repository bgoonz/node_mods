  ...
  validateConstraints(mapper: Mapper, value: any, objectName: string): void {
    ...
    if (mapper.constraints && value != undefined) {
      ...
      if (
        UniqueItems &&
        value.some(
          (item: any, i: number, ar: Array<any>) => ar.indexOf(item) !== i
        )
      ) {
  ...
  if (object != undefined) {
        ...
        if (serializedValue !== undefined && propName != undefined) {
          if (propertyMapper.xmlIsAttribute) {
            ...
            parentObject[propName] = {
              [propertyMapper.xmlElementName!]: serializedValue,
            };
          } else {
            parentObject[propName] = serializedValue;
          }
    ...
    if (additionalPropertiesMapper) {
      ...
      for (const clientPropName in object) {
        const isAdditionalProperty = propNames.every(
          (pn) => pn !== clientPropName
        );
...
function deserializeCompositeType(
  ...
  for (const key of Object.keys(modelProps)) {
    ...
    const headerCollectionPrefix = (propertyMapper as DictionaryMapper)
