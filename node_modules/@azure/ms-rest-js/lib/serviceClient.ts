...
export interface ServiceClientOptions {
  ...
  userAgentHeaderName?:
    | string
    | ((defaultUserAgentHeaderName: string) => string);
  /**
...
export class ServiceClient {
  ...
  sendOperationRequest(
    ...
    try {
        ...
        for (const headerParameter of operationSpec.headerParameters) {
          ...
          if (headerValue != undefined) {
            ...
            const headerCollectionPrefix = (
              headerParameter.mapper as DictionaryMapper
      ...
      const options: RequestOptionsBase | undefined =
  ...
  if (operationSpec.requestBody && operationSpec.requestBody.mapper) {
    ...
    try {
      if (httpRequest.body != undefined || required) {
        const requestBodyParameterPathString: string =
          getPathStringFromParameter(operationSpec.requestBody);
    ...
    for (const formDataParameter of operationSpec.formDataParameters) {
      const formDataParameterValue: any =
        getOperationArgumentValueFromParameter(
          serviceClient,
          operationArguments,
          formDataParameter,
...
function isRequestPolicyFactory(
  instance: any
): instance is RequestPolicyFactory {
 ...
 * parent object.
 */
export function getPropertyParent(
  ...
  if (Array.isArray(parameterPath)) {
    if (parameterPath.length > 0) {
      if (parameterMapper.isConstant) {
        ...
        let propertySearchResult: PropertySearchResult =
          getPropertyFromParameterPath(operationArguments, parameterPath);
    ...
    for (const propertyName in parameterPath) {
      const propertyMapper: Mapper = (parameterMapper as CompositeMapper).type
