...
var handleRequests = function handleRequests(userConfig) {
  ...
  return {
    ...
    requestsMiddleware: [
      config.ssr !== "server" &&
        config.subscriber &&
        (0, _middleware.createSubscriptionsMiddleware)(config),
      config.ssr !== "server" &&
        (0, _middleware.createPollingMiddleware)(config),
      config.ssr === "server" &&
        !config.disableRequestsPromise &&
        (0, _middleware.createServerSsrMiddleware)(requestsPromise, config),
      config.ssr === "client" &&
        (0, _middleware.createClientSsrMiddleware)(config),
      config.cache && (0, _middleware.createRequestsCacheMiddleware)(config),
      (0, _middleware.createSendRequestsMiddleware)(config),
    ].filter(Boolean),
