...
class ModelFactoryKey<TModelOptions> {
  ...
  public static fromString<TModelOptions>(
    key: string
  ): ModelFactoryKey<TModelOptions> {
...
export class ModelFactory<TModelOptions> {
  ...
  private modelFactories: Map<string, (options: TModelOptions) => IModel> =
    new Map<string, (options: TModelOptions) => IModel>();
  ...
  getModel(
    ...
    culture: string,
    fallbackToDefaultCulture: boolean,
    options: TModelOptions
  ): IModel {
    ...
    throw new Error(
      `Could not find Model with the specified configuration: ${culture},${modelTypeName}`
    );
  }
  ...
  tryGetModel(
    ...
    culture: string,
    options: TModelOptions
  ): { containsModel: boolean; model?: IModel } {
  ...
  registerModel(
    modelTypeName: string,
    culture: string,
    modelCreator: (options: TModelOptions) => IModel
  ) {
    let key = this.generateKey(modelTypeName, culture);
    if (this.modelFactories.has(key)) {
      throw new Error(
        `${culture}-${modelTypeName} has already been registered.`
      );
    }
  ...
  initializeModels(targetCulture: string, options: TModelOptions) {
    this.modelFactories.forEach((value, key) => {
      ...
      if (
        StringUtility.isNullOrEmpty(targetCulture) ||
  ...
  private generateCacheKey(
    ...
    return new ModelFactoryKey<TModelOptions>(
      culture,
      modelTypeName,
      options
    ).toString();
