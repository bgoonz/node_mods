...
class RegExpUtility {
  static getMatches(regex, source) {
    ...
    while (startPos >= 0) {
      ...
      nlbRegex.nextRegex = nextRegex ? XRegExp(nextRegex, flags) : null;
  ...
  static sanitizeGroups(source) {
    let index = 0;
    let result = XRegExp.replace(source, this.matchGroup, (match, name) =>
      match.replace(name, `${name}__${index++}`)
    );
    index = 0;
    result = XRegExp.replace(
      result,
...
RegExpUtility.matchGroup = XRegExp(String.raw`\?<(?<name>\w+)>`, "gis");
RegExpUtility.matchPositiveLookbehind = XRegExp(String.raw`\(\?<=`, "gis");
RegExpUtility.matchNegativeLookbehind = XRegExp(String.raw`\(\?<!`, "gis");
