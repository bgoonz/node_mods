export declare namespace EnglishNumeric {
  ...
  const RoundNumberOrdinalRegex =
    "(hundredth|thousandth|millionth|billionth|trillionth)";
  const BasicOrdinalRegex =
  ...
  const OrdinalNumericRegex =
    "(?<=\\b)(\\d{1,3}(\\s*,\\s*\\d{3})*\\s*th)(?=\\b)";
  ...
  const DoubleWithMultiplierRegex =
    "(((?<!\\d+\\s*)-\\s*)|((?<=\\b)(?<!\\d+[\\.,])))\\d+[\\.,]\\d+\\s*(K|k|M|G|T|B|b)(?=\\b)";
  ...
  const MoreRegex =
    "((bigger|greater|more|higher|larger)(\\s+than)?|above|over|(?<!<|=)>)";
  const LessRegex =
    "((less|lower|smaller|fewer)(\\s+than)?|below|under|(?<!>|=)<)";
  const EqualRegex = "(equal(s|ing)?(\\s+(to|than))?|(?<!<|>)=)";
  const MoreOrEqualPrefix: string;
  const MoreOrEqual: string;
  const MoreOrEqualSuffix =
    "((and|or)\\s+(more|greater|higher|larger|bigger)((?!\\s+than)|(\\s+than(?!(\\s*\\d+)))))";
  const LessOrEqualPrefix: string;
  const LessOrEqual: string;
  const LessOrEqualSuffix =
    "((and|or)\\s+(less|lower|smaller|fewer)((?!\\s+than)|(\\s+than(?!(\\s*\\d+)))))";
  const NumberSplitMark = "(?![,.](?!\\d+))";
  const MoreRegexNoNumberSucceed =
    "((bigger|greater|more|higher|larger)((?!\\s+than)|\\s+(than(?!(\\s*\\d+))))|(above|over)(?!(\\s*\\d+)))";
  const LessRegexNoNumberSucceed =
    "((less|lower|smaller|fewer)((?!\\s+than)|\\s+(than(?!(\\s*\\d+))))|(below|under)(?!(\\s*\\d+)))";
  const EqualRegexNoNumberSucceed =
    "(equal(s|ing)?((?!\\s+(to|than))|(\\s+(to|than)(?!(\\s*\\d+)))))";
