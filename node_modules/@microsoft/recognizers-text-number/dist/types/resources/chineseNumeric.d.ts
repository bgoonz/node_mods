export declare namespace ChineseNumeric {
  ...
  const OneNumberRangeMoreRegex2 =
    "比\\s*(?<number1>((?!((，(?!\\d+))|(,(?!\\d+))|。)).)+)\\s*更?[大多高]";
  const OneNumberRangeMoreRegex3 =
    "(?<number1>((?!((，(?!\\d+))|(,(?!\\d+))|。|[或者])).)+)\\s*(或|或者)?\\s*([多几余幾餘]|以上|之上|更[大多高])(?![万亿萬億]{1,2})";
  const OneNumberRangeLessRegex1: string;
  const OneNumberRangeLessRegex2 =
    "比\\s*(?<number2>((?!((，(?!\\d+))|(,(?!\\d+))|。)).)+)\\s*更?[小少低]";
  const OneNumberRangeLessRegex3 =
    "(?<number2>((?!((，(?!\\d+))|(,(?!\\d+))|。|[或者])).)+)\\s*(或|或者)?\\s*(以下|之下|更[小少低])";
