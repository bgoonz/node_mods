export declare namespace SpanishDateTime {
  ...
  const RelativeMonthRegex =
    "(?<relmonth>(este|pr[oó]ximo|[uú]ltimo)\\s+mes)\\b";
  const MonthRegex =
  ...
  const DateUnitRegex =
    "(?<unit>años|año|meses|mes|semanas|semana|d[ií]a(s)?)\\b";
  const PastRegex =
    "(?<past>\\b(pasad(a|o)(s)?|[uú]ltim[oa](s)?|anterior(es)?|previo(s)?)\\b)";
  const FutureRegex =
    "(?<past>\\b(siguiente(s)?|pr[oó]xim[oa](s)?|dentro\\s+de|en)\\b)";
  ...
  const HourNumRegex =
    "\\b(?<hournum>cero|una|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez|once|doce)\\b";
  ...
  const DeltaMinuteNumRegex =
    "(?<deltaminnum>un|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez|once|doce|trece|catorce|quince|dieciseis|diecisiete|dieciocho|diecinueve|veinte|treinta|cuarenta|cincuenta)";
  ...
  const TimeRegex10 =
    "(a\\s+la|al)\\s+(madrugada|mañana|medio\\s*d[ií]a|tarde|noche)";
  const TimeRegex11: string;
  const TimeRegex12: string;
  const PrepositionRegex = "(?<prep>(a(l)?|en|de(l)?)?(\\s*(la(s)?|el|los))?$)";
  const NowRegex =
    "\\b(?<now>(justo\\s+)?ahora(\\s+mismo)?|en\\s+este\\s+momento|tan\\s+pronto\\s+como\\s+sea\\s+posible|tan\\s+pronto\\s+como\\s+(pueda|puedas|podamos|puedan)|lo\\s+m[aá]s\\s+pronto\\s+posible|recientemente|previamente)\\b";
  const SuffixRegex =
    "^\\s*(((y|a|en|por)\\s+la|al)\\s+)?(mañana|madrugada|medio\\s*d[ií]a|tarde|noche)\\b";
  const TimeOfDayRegex =
    "\\b(?<timeOfDay>mañana|madrugada|(pasado\\s+(el\\s+)?)?medio\\s?d[ií]a|tarde|noche|anoche)\\b";
  ...
  const TheEndOfRegex =
    "((a|e)l\\s+)?fin(alizar|al)?(\\s+(el|de(l)?)(\\s+d[ií]a)?(\\s+de)?)?\\s*$";
  const UnitRegex =
  ...
  const PeriodTimeOfDayWithDateRegex =
    "\\b(((y|a|en|por)\\s+la|al)\\s+)?(?<timeOfDay>mañana|madrugada|(pasado\\s+(el\\s+)?)?medio\\s?d[ií]a|tarde|noche|anoche)\\b";
  const RelativeTimeUnitRegex: string;
  const LessThanRegex = "^[.]";
  const MoreThanRegex = "^[.]";
  const SuffixAndRegex =
    "(?<suffix>\\s*(y)\\s+((un|uno|una)\\s+)?(?<suffix_num>media|cuarto))";
  const FollowedUnit: string;
  const DurationNumberCombinedWithUnit: string;
  const AnUnitRegex: string;
  const DuringRegex = "^[.]";
  const AllRegex =
    "\\b(?<all>tod[oa]?\\s+(el|la)\\s+(?<unit>año|mes|semana|d[ií]a))\\b";
  const HalfRegex =
    "\\b(?<half>medi[oa]\\s+(?<unit>ano|mes|semana|d[íi]a|hora))\\b";
  ...
  const PrepositionSuffixRegex =
    "\\b(on|in|at|around|for|during|since|from|to)$";
