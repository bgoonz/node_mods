...
describe("email", () => {
  ...
  describe("analyze()", () => {
    ...
    it("validates options", () => {
      ...
      for (let i = 0; i < tests.length; ++i) {
        const email = tests[i];
        expect(() => Address.email.analyze(email[0], email[2])).to.throw(
          email[1]
        );
      }
    });
    ...
    describe("validated TLD", () => {
      it("applies built-in list", () => {
        ...
        expect(
          Address.email.analyze("test@example.com", { tlds: true })
        ).to.not.exist();
        expect(
          Address.email.analyze("test@example.com", { tlds: { allow: true } })
        ).to.not.exist();
      });

      it("ignores built-in list", () => {
        expect(
          Address.email.analyze("test@example.invalid-top", { tlds: false })
        ).to.not.exist();
      });

      it("denies listed tls", () => {
        expect(
          Address.email.analyze("test@example.com", {
            tlds: { deny: new Set(["test"]) },
          })
        ).to.not.exist();
        expect(
          Address.email.analyze("test@example.com", {
            tlds: { deny: new Set(["com"]) },
          })
        ).to.equal({ error: "Domain uses forbidden TLD" });
      });
    });
  });
  ...
  describe("isValid()", () => {
    it("validates email", () => {
      ...
      const tests = [
        ...
        [
          "test@abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghiklm",
          false,
        ],
        [
          "test@\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06\ud83d\ude06.org",
...
describe("domain", () => {
  ...
  describe("analyze()", () => {
    ...
    it("validates options", () => {
      ...
      for (let i = 0; i < tests.length; ++i) {
        const domain = tests[i];
        expect(() => Address.domain.analyze(domain[0], domain[2])).to.throw(
          domain[1]
        );
      }
    });
