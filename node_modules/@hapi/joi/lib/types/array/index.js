...
internals.Array = class extends Any {
  ...
  _base(value, state, options) {
    ...
    if (
      ...
      if (result.errors && wasArray && options.convert && this._flags.single) {
        ...
        result.value = [result.value];
        result.errors = this._checkItems(
          result.value,
          wasArray,
          state,
          options
  ...
  _checkItems(items, wasArray, state, options) {
    ...
    for (let i = 0; i < il; ++i) {
      ...
      for (let j = 0; j < jl; ++j) {
        res = requiredChecks[j] = requireds[j]._validate(
          item,
          localState,
          options
        );
      ...
      const stripUnknown =
        (options.stripUnknown && !!options.stripUnknown.arrays) || false;
      ...
      for (let j = 0; j < jl; ++j) {
        ...
        if (jl === 1) {
          ...
          errors.push(
            this.createError(
              wasArray ? "array.includesOne" : "array.includesOneSingle",
              { pos: i, reason: res.errors, value: item },
              { key: state.key, path: localState.path },
              options
            )
  ...
  unique(comparator, configs) {
    ...
    Hoek.assert(
      configs === undefined || typeof configs === "object",
      ...
      for (let i = 0; i < value.length; ++i) {
        ...
        if (/* $lab:coverage:off$ */ records /* $lab:coverage:on$ */) {
          if (records instanceof Map) {
            ...
            if (
              (!ignoreUndefined || item !== undefined) &&
