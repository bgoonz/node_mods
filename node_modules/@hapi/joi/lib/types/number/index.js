    ...
    if (typeof value === "string" && options.convert) {
      ...
      if (matches) {
        ...
        if (!this._flags.unsafe) {
          if (value.includes("e")) {
            if (
              internals.normalizeExponent(
                `${result.value / Math.pow(10, matches[1])}e${matches[1]}`
              ) !== internals.normalizeExponent(value)
            ) {
  ...
  integer() {
    return this._test("integer", undefined, function (value, state, options) {
      return Math.trunc(value) - value === 0
        ? value
        : this.createError("number.integer", { value }, state, options);
    });
  }
