...
internals.String = class extends Any {
  ...
  _base(value, state, options) {
    ...
    return {
      ...
      errors:
        typeof value === "string"
          ? null
          : this.createError("string.base", { value }, state, options),
    };
  }
  ...
  creditCard() {
    return this._test(
      ...
      function (value, state, options) {
        ...
        return check
          ? value
          : this.createError("string.creditCard", { value }, state, options);
      }
    );
  }
  ...
  regex(pattern, patternOptions) {
    ...
    Hoek.assert(
      !pattern.flags.includes("g") && !pattern.flags.includes("y"),
    ...
    return this._test("regex", patternObject, function (value, state, options) {
      ...
      return this.createError(
        errorCode,
        { name: patternObject.name, pattern: patternObject.pattern, value },
        state,
        options
      );
    });
  ...
  email(validationOptions) {
    if (validationOptions) {
      ...
      Hoek.assert(
        validationOptions.checkDNS === undefined,
        "checkDNS option is not supported"
      );
      Hoek.assert(
        validationOptions.errorLevel === undefined,
        "errorLevel option is not supported"
      );
      Hoek.assert(
        validationOptions.minDomainAtoms === undefined,
        "minDomainAtoms option is not supported, use minDomainSegments instead"
      );
      Hoek.assert(
        validationOptions.tldBlacklist === undefined,
        "tldBlacklist option is not supported, use tlds.deny instead"
      );
      Hoek.assert(
        validationOptions.tldWhitelist === undefined,
        "tldWhitelist option is not supported, use tlds.allow instead"
      );
      ...
      if (
        ...
        Hoek.assert(
          validationOptions.tlds.allow === undefined ||
            validationOptions.tlds.allow === false ||
            validationOptions.tlds.allow === true ||
            Array.isArray(validationOptions.tlds.allow) ||
            validationOptions.tlds.allow instanceof Set,
          "tlds.allow must be an array, Set, or boolean"
        );

        Hoek.assert(
          validationOptions.tlds.deny === undefined ||
            Array.isArray(validationOptions.tlds.deny) ||
            validationOptions.tlds.deny instanceof Set,
          "tlds.deny must be an array or Set"
        );
      ...
      Hoek.assert(
        validationOptions.minDomainSegments === undefined ||
          (Number.isSafeInteger(validationOptions.minDomainSegments) &&
            validationOptions.minDomainSegments > 0),
  ...
  ip(ipOptions = {}) {
    ...
    if (ipOptions.cidr) {
      ...
      Hoek.assert(
        Hoek.contain(internals.cidrPresences, ipOptions.cidr),
    ...
    return this._test("ip", ipOptions, function (value, state, options) {
      ...
      if (versions) {
        return this.createError(
          "string.ipVersion",
          { value, cidr: ipOptions.cidr, version: versions },
          state,
          options
        );
      }
  ...
  uri(uriOptions) {
    ...
    if (uriOptions) {
      ...
      const unknownOptions = Object.keys(uriOptions).filter(
        (key) =>
          ![
            "scheme",
            "allowRelative",
        ...
        for (let i = 0; i < uriOptions.scheme.length; ++i) {
          ...
          if (scheme instanceof RegExp) {
            ...
            Hoek.assert(
              ...
              "scheme at position " + i + " must be a valid scheme"
            );
    ...
    if (guidOptions && guidOptions.version) {
      ...
      for (let i = 0; i < guidOptions.version.length; ++i) {
        ...
        Hoek.assert(
          ...
          "version at position " + i + " must not be a duplicate."
        );
  ...
  hex(hexOptions = {}) {
    Hoek.assert(
      typeof hexOptions === "object",
      "hex options must be an object"
    );
    Hoek.assert(
      typeof hexOptions.byteAligned === "undefined" ||
        typeof hexOptions.byteAligned === "boolean",
  ...
  dataUri(dataUriOptions = {}) {
    ...
    // Determine if padding is required.
    const paddingRequired =
      dataUriOptions.paddingRequired === false
  ...
  normalize(form = "NFC") {
    Hoek.assert(
      Hoek.contain(internals.normalizationForms, form),
      "normalization form must be one of " +
        internals.normalizationForms.join(", ")
    );
...
internals.compare = function (type, compare) {
  return function (limit, encoding) {
    ...
    return this._test(type, limit, function (value, state, options) {
      ...
      return this.createError(
        "string." + type,
        { limit: compareTo, value, encoding },
        state,
        options
      );
    });
