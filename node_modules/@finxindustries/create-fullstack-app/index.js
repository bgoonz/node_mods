...
if (args.length === 0 || args[0] === "." || args[0] === "./") {
  ...
  if (!/^[a-z_]+$/.test(projectName)) {
    console.log(
      chalk.red(
        `${projectName} is not an valid project name! Only lowercase letters and _ are allowed!`
      )
    );
    return 1;
  }
...
projectMobileReverseDomain = prompt(
  "Mobile Reverse Domain (ex: com.example): "
);
if (
  !/^[a-z.]+$/.test(projectMobileReverseDomain) ||
  projectMobileReverseDomain === ""
) {
  console.log(
    chalk.red(`${projectName} is not an valid reverse domain! ex: com.example`)
  );
  return 1;
}

var answer = prompt(
  "Do you have dotnet, flutter, make, docker and nodejs installed? y/n: "
);
...
if (!commandExists("dotnet")) {
  console.log(
    chalk.red(
      "dotnet missing in PATH, please install dotnet sdk: https://dotnet.microsoft.com/download/dotnet-core/3.1"
    )
  );
  failed = true;
}
if (!commandExists("flutter")) {
  console.log(
    chalk.red(
      "flutter missing in PATH, please install flutter sdk: https://flutter.dev/docs/get-started/install"
    )
  );
  failed = true;
}
  ...
  files.forEach((file) => {
    ...
    if (/____templatePlaceholderMRDExtension____/.test(file)) {
      let name = file.replace(
        "____templatePlaceholderMRDExtension____",
        projectMobileReverseDomain.split(".")[0]
      );
      let src = path.join(dir, file);
      let newSrc = path.join(dir, name);
      fileList.push({
    ...
    if (/____templatePlaceholderMRDName____/.test(file)) {
      let name = file.replace(
        "____templatePlaceholderMRDName____",
        projectMobileReverseDomain.split(".")[1]
      );
      let src = path.join(dir, file);
      let newSrc = path.join(dir, name);
      fileList.push({
...
foundFilesReplaceText.forEach((f) => {
  ...
  try {
    ...
    if (results[0].hasChanged) console.log("Configured: " + f);
