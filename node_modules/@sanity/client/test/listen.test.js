  ...
  new Promise((resolve, reject) => {
    sseServer(onRequest, (err, server) => {
      ...
      const client = getClient(
        assign({ port: server.address().port }, options)
      );
...
test("[listener] emits channel errors", (t) => {
  testSse(({ request, channel }) => {
    channel.send({
      event: "channelError",
      data: { message: "Unfortunate error" },
    });
    channel.close();
    process.nextTick(() => channel.close());
  })
      ...
      const subscription = client.listen("*").subscribe({
        error: (err) => {
          t.equal(
            err.message,
            "Unfortunate error",
            "should have passed error message"
          );
          subscription.unsubscribe();
          server.close();
          t.end();
...
test("[listener] emits channel errors with deep error description", (t) => {
  testSse(({ request, channel }) => {
    channel.send({
      event: "channelError",
      data: { error: { description: "Expected error" } },
    });
    channel.close();
    process.nextTick(() => channel.close());
  })
