...
test("handles api errors gracefully", (t) => {
  ...
  getClient()
    ...
    .catch((err) => {
      ...
      t.ok(
        err.message.includes(response.message),
        "should contain error message"
      );
      t.ok(
        err.responseBody.includes(response.message),
        "responseBody should be populated"
      );
      t.end();
    });
});
...
test("handles db errors gracefully", (t) => {
  ...
  getClient()
    ...
    .catch((err) => {
      t.ok(err instanceof Error, "should be error");
      t.ok(
        err.message.includes(response.error.description),
        "should contain error description"
      );
      t.equal(
        err.details.column,
        response.error.column,
        "error should have details object"
      );
      t.equal(
        err.details.line,
        response.error.line,
...
test("populates response body on errors", (t) => {
  ...
  getClient()
    ...
    .catch((err) => {
      ...
      t.ok(err.message.includes("HTTP 400"), "should contain status code");
      t.ok(
        (err.responseBody || "").includes("Some Weird Error"),
        "body populated"
      );
      t.end();
    });
...
test("can tell create() not to return documents", (t) => {
  const doc = { _id: "abc123", name: "Raptor" };
  nock(projectHost())
    .post("/v1/data/mutate/foo?returnIds=true&visibility=sync", {
      mutations: [{ create: doc }],
    })
    .reply(200, {
      transactionId: "abc123",
      results: [{ id: "abc123", operation: "create" }],
    });
...
test("can tell create() to use non-default visibility mode", (t) => {
  ...
  nock(projectHost())
    .post(
      "/v1/data/mutate/foo?returnIds=true&returnDocuments=true&visibility=async",
      {
        mutations: [{ create: doc }],
      }
    )
...
test("can use request tag with createIfNotExists()", (t) => {
  ...
  nock(projectHost())
    .post(
      "/v1/data/mutate/foo?tag=mysync&returnIds=true&visibility=sync",
      expectedBody
    )
    .reply(200, {
      transactionId: "abc123",
...
test("delete() sends correct mutation", (t) => {
  ...
  nock(projectHost())
    ...
    .reply(200, {
      transactionId: "abc123",
      results: [{ id: "abc123", operation: "delete" }],
    });
...
test("mutate() accepts multiple mutations", (t) => {
  ...
  nock(projectHost())
    ...
    .reply(200, {
      ...
      results: [
        {
          id: "movies.raiders-of-the-lost-ark",
          operation: "create",
          document: docs[0],
        },
        {
          id: "movies.the-phantom-menace",
          operation: "delete",
          document: docs[1],
        },
      ],
...
test("mutate() accepts request tag", (t) => {
  ...
  nock(projectHost())
    ...
    .reply(200, {
      transactionId: "foo",
      results: [
        { id: "abc123", operation: "delete", document: { _id: "abc123" } },
      ],
    });
...
test("setIfMissing() patch can be applied multiple times", (t) => {
  ...
  t.deepEqual(patch, {
    id: "abc123",
    setIfMissing: { count: 2, foo: "bar", bar: "foo" },
  });
  t.end();
});
...
test("can apply splice()", (t) => {
  ...
  t.deepEqual(
    negativeDelete,
    patch().splice("tags", -2, 0, ["foo"]).serialize()
  );
  t.end();
});
...
test("executes patch when commit() is called", (t) => {
  const expectedPatch = {
    patch: { id: "abc123", inc: { count: 1 }, set: { visited: true } },
  };
  nock(projectHost())
    .post("/v1/data/mutate/foo?returnIds=true&visibility=sync", {
...
test("executes patch with request tag when commit() is called with tag", (t) => {
  ...
  nock(projectHost())
    .post(
      "/v1/data/mutate/foo?tag=company.setvisited&returnIds=true&visibility=sync",
      {
        mutations: [expectedPatch],
      }
    )
...
test("executes transaction with request tag when commit() is called with tag", (t) => {
  ...
  nock(projectHost())
    .post(
      "/v1/data/mutate/foo?tag=sfcraft.createbar&returnIds=true&visibility=sync",
      { mutations }
    )
    .reply(200, { transactionId: "blatti" });
...
test("uploads images", (t) => {
  ...
  const isImage = (body) =>
    bufferFrom(body, "hex").compare(fs.readFileSync(fixturePath)) === 0;
...
test("delete assets", (t) => {
  const expectedBody = {
    mutations: [{ delete: { id: "image-abc123_foobar-123x123-png" } }],
  };
  nock(projectHost())
    .post(
      "/v1/data/mutate/foo?returnIds=true&returnDocuments=true&visibility=sync",
...
test("delete assets given whole asset document", (t) => {
  ...
  const doc = {
    _id: "image-abc123_foobar-123x123-png",
...
test("can get an image URL from a reference ID string", (t) => {
  ...
  t.equal(
    url,
    "https://cdn.sanity.io/images/bf1942/foo/someImageId-200x300.png"
  );
  t.end();
});

test("can get an image URL from a reference object", (t) => {
  const url = getClient().assets.getImageUrl({
...
test("ClientError includes message in stack", (t) => {
  ...
  const error = new SanityClient.ClientError({
    statusCode: 400,
    headers: {},
    body,
  });
...
test("exposes ServerError", (t) => {
  ...
  const error = new SanityClient.ServerError({
    statusCode: 500,
    headers: {},
    body: {},
  });
  ...
  getClient({ timeout: 150 })
    ...
    .catch((err) => {
      ...
      t.equal(
        err.code,
        "ETIMEDOUT",
        `should have timeout error code, got err\n${err.toString()}`
      );
      t.end();
    });
});
