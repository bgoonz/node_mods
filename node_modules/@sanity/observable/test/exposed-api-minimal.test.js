...
function run(t) {
  ...
  test("static properties", (t) => {
    const staticFields = staticProps.filter(
      (key) => typeof Observable[key] !== "function"
    );
    t.ok(
  ...
  test("static methods", (t) => {
    const staticFunctions = staticProps.filter(
      (key) => typeof Observable[key] === "function"
    );
...
test(
  'Exposed api should not be affected by require("rxjs")',
  { autoend: true },
  (t) => {
    require("rxjs");
    run(t);
