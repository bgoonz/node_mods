...
class BufferedDocument {
  ...
  constructor(doc) {
    ...
    this.document.onRemoteMutation = (mut) =>
      this.onRemoteMutation && this.onRemoteMutation(mut);

    this.document.onRebase = (msg, remoteMutations, localMutations) =>
      this.handleDocRebase(msg, remoteMutations, localMutations);

    this.document.onConsistencyChanged = (msg) =>
      this.handleDocConsistencyChanged(msg);
  ...
  arrive(mutation) {
    (0, _debug.default)(
      "Remote mutation arrived %s -> %s",
      mutation.previousRev,
      mutation.resultRev
    );
    ...
    if (mutation.previousRev == mutation.resultRev) {
      throw new Error(
        "Mutation "
          .concat(mutation.transactionId, " has previousRev == resultRev (")
          .concat(mutation.previousRev, ")")
      );
    }
  ...
  rebase(remoteMutations, localMutations) {
    ...
    var oldLocal = this.LOCAL;
    this.LOCAL = this.commits.reduce(
      (doc, commit) => commit.apply(doc),
      this.document.EDGE
    );
