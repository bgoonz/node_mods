...
test("Expression constraints", (tap) => {
  ...
  tap.equal(
    false,
    attrCompare.testConstraint(new PlainProbe({ banana: "nice" }))
  );
  tap.equal(
    true,
    attrCompare.testConstraint(new PlainProbe({ banana: "rotten" }))
  );
  ...
  tap.equal(
    false,
    numCompare.testConstraint(new PlainProbe({ number: "123" }))
  );
  ...
  tap.equal(
    false,
    strNumCompare.testConstraint(new PlainProbe({ number: 123 }))
  );
  tap.equal(
    true,
    strNumCompare.testConstraint(new PlainProbe({ number: "123" }))
...
test("Expression toIndicies", (tap) => {
  ...
  tap.same(
    [2],
    index.toIndicies(new PlainProbe(["a", "a", "a", "a", "a", "a", "a", "a"]))
  );
  tap.end();
});
