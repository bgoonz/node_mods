...
test("Match attribute constraint on array", (tap) => {
  const d = new Descender(
    new Expression(inner("[a == 7]")),
    Expression.fromPath("[b,c].d")
  );
...
test((tap) => {
  const notRecursive = new Descender(
    Expression.fromPath("key"),
    Expression.fromPath("banana")
  );
  tap.equal(false, notRecursive.isRecursive());
  const recursive = new Descender(
    Expression.fromPath("..key"),
    Expression.fromPath("banana")
  );
...
test((tap) => {
  const recursive = new Descender(
    Expression.fromPath("..a"),
    Expression.fromPath("b.c")
  );
