...
function assertNoStashedOperations(tap, sb) {
  tap.assert(
    sb.out.length === 0,
    "There should not be any stashed changes yet"
  );
}

test("basic optimization of assignments to same, explicit key", (tap) => {
  const sb = new SquashingBuffer({
    _id: "1",
    _type: "test",
    a: "A string value",
  });
  ...
  tap.same(
    ...
    ["a"],
    "Should still only be one patch"
  );
...
test("rebase with no local edits", (tap) => {
  ...
  tap.false(
    mut,
    "purge should not return anything when there are no local changes"
  );
  tap.same(
    sb.PRESTAGE,
    {
