...
function generateProxyingResolvers(subschemaConfig) {
  ...
  const transformedSchema = delegate.applySchemaTransforms(
    targetSchema,
    subschemaConfig
  );
      ...
      Object.keys(fields).forEach((fieldName) => {
        ...
        if (operation === "subscription") {
          resolvers[typeName][fieldName] = {
            ...
            resolve: (payload, _, __, { fieldName: targetFieldName }) =>
...
class TransformCompositeFields {
  ...
  transformResult(result, _delegationContext, transformationContext) {
    if (this.dataTransformer != null) {
      result.data = utils.visitData(result.data, (value) =>
        this.dataTransformer(value, transformationContext)
      );
    }
  ...
  constructor(renamer) {
    this.transformer = new TransformRootFields(
      (operation, fieldName, fieldConfig) => [
        renamer(operation, fieldName, fieldConfig),
        fieldConfig,
      ]
    );
  }
...
class TransformInputObjectFields {
  ...
  transformSchema(
    ...
    this.transformedSchema = utils.mapSchema(originalWrappingSchema, {
      [utils.MapperKind.INPUT_OBJECT_FIELD]: (
        inputFieldConfig,
        fieldName,
        typeName
      ) => {
          ...
          variableValues[varName] = utils.transformInputValue(
            ...
            (type, originalValue) => {
              ...
              Object.keys(originalValue).forEach((key) => {
                ...
                if (field != null) {
                  const newFieldName =
                    (_a = this.mapping[type.name]) === null || _a === void 0
                      ? void 0
                      : _a[field.name];
...
class FilterInputObjectFields {
  constructor(filter, inputObjectNodeTransformer) {
    this.transformer = new TransformInputObjectFields(
      (typeName, fieldName, inputFieldConfig) =>
...
class MapLeafValues {
    ...
    Object.keys(typeMap).forEach((typeName) => {
      ...
      if (!typeName.startsWith("__")) {
        if (graphql.isLeafType(type)) {
          this.resultVisitorMap[typeName] = (value) =>
  ...
  transformOperations(operations, variableValues) {
    return operations.map((operation) => {
      ...
      const newOperation = graphql.visit(
        operation,
        graphql.visitWithTypeInfo(this.typeInfo, {
          [graphql.Kind.FIELD]: (node) =>
            this.transformFieldNode(
              node,
      ...
      if (argumentNodes != null) {
        ...
        targetField.args.forEach((argument) => {
          ...
          utils.updateArgument(
            ...
            variableDefinitionMap,
            variableValues,
            utils.transformInputValue(argType, value, (t, v) => {
...
function generateValueTransformer(valueTransformer, mapping) {
  if (valueTransformer == null) {
    ...
    return (typeName, value) =>
      mapEnumValues(typeName, valueTransformer(typeName, value), mapping);
  }
}
    ...
    const transformer = new TransformObjectFields(
      (_typeName, _fieldName, fieldConfig) => {
        const keepDirectives = fieldConfig.astNode.directives.filter((dir) => {
          ...
          const directiveValue = directiveDef
...
class RemoveObjectFieldsWithDirective {
  ...
  transformSchema(originalWrappingSchema, subschemaConfig, transformedSchema) {
    const transformer = new FilterObjectFields(
      (_typeName, _fieldName, fieldConfig) => {
        const valueMap = utils.getDirectives(
          originalWrappingSchema,
          fieldConfig
        );
        return !Object.keys(valueMap).some(
          (directiveName) =>
            utils.valueMatchesCriteria(directiveName, this.directiveName) &&
            ((Array.isArray(valueMap[directiveName]) &&
              valueMap[directiveName].some((value) =>
                utils.valueMatchesCriteria(value, this.args)
              )) ||
    ...
    const wrappingRootField =
      this.outerTypeName ===
        ((_a = originalWrappingSchema.getQueryType()) === null || _a === void 0
          ...
          : _a.name) ||
      this.outerTypeName ===
        ((_b = originalWrappingSchema.getMutationType()) === null ||
        _b === void 0
          ? void 0
          : _b.name);
    let resolve;
      ...
      if (wrappingRootField) {
        ...
        const operation =
          this.outerTypeName === targetSchema.getQueryType().name
            ...
            : "mutation";
        const createProxyingResolver =
          (_c = subschemaConfig.createProxyingResolver) !== null &&
          _c !== void 0
            ? _c
            : defaultCreateProxyingResolver;
  ...
  transformSchema(originalWrappingSchema, subschemaConfig, transformedSchema) {
    ...
    if (transformedSchema) {
      const hoistingToRootField =
        this.typeName ===
          ((_a = originalWrappingSchema.getQueryType()) === null ||
          _a === void 0
            ? void 0
            : _a.name) ||
        this.typeName ===
...
function wrapFieldNode(fieldNode, path, alias, argLevels) {
  return path.reduceRight(
    (acc, fieldName, index) => ({
      ...
      arguments: fieldNode.arguments.filter(
