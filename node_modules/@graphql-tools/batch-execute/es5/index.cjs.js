...
function mergeExecutionParams(execs, extensionsReducer) {
  ...
  execs.forEach(function (executionParams, index) {
    var prefixedExecutionParams = prefixExecutionParams(
      createPrefix(index),
      executionParams
    );
    prefixedExecutionParams.document.definitions.forEach(function (def) {
      ...
      if (isOperationDefinition(def)) {
        ...
        mergedSelections.push.apply(
          mergedSelections,
          tslib.__spreadArray([], tslib.__read(def.selectionSet.selections))
        );
        mergedVariableDefinitions.push.apply(
          mergedVariableDefinitions,
          tslib.__spreadArray(
            [],
            tslib.__read(
              (_a = def.variableDefinitions) !== null && _a !== void 0 ? _a : []
            )
          )
        );
      }
  ...
  return {
    document: {
      ...
      definitions: tslib.__spreadArray(
        ...
        tslib.__read(mergedFragmentDefinitions)
      ),
    },
...
function prefixExecutionParams(prefix, executionParams) {
  ...
  if (variableNames.length === 0) {
    return tslib.__assign(tslib.__assign({}, executionParams), {
      document: document,
    });
  }
...
function aliasTopLevelFields(prefix, document) {
  ...
  var transformer =
    ...
    (_a[graphql.Kind.OPERATION_DEFINITION] = function (def) {
      ...
      return tslib.__assign(tslib.__assign({}, def), {
        selectionSet: tslib.__assign(tslib.__assign({}, def.selectionSet), {
  ...
  return graphql.visit(
    document,
    transformer,
    ((_b = {}), (_b[graphql.Kind.DOCUMENT] = ["definitions"]), _b)
  );
}
/**
...
function aliasFieldsInInlineFragment(prefix, fragment, document) {
  ...
  return tslib.__assign(tslib.__assign({}, fragment), {
    selectionSet: tslib.__assign(tslib.__assign({}, fragment.selectionSet), {
      selections: aliasFieldsInSelection(prefix, selections, document),
    }),
  });
}
...
function inlineFragmentSpread(spread, document) {
  var fragment = document.definitions.find(function (def) {
    return isFragmentDefinition(def) && def.name.value === spread.name.value;
  });
  if (!fragment) {
...
function prefixNodeName(namedNode, prefix) {
  return tslib.__assign(tslib.__assign({}, namedNode), {
    name: tslib.__assign(tslib.__assign({}, namedNode.name), {
...
function aliasField(field, aliasPrefix) {
  ...
  return tslib.__assign(tslib.__assign({}, field), {
    alias: tslib.__assign(tslib.__assign({}, aliasNode), {
...
function splitResult(mergedResult, numResults) {
  ...
  if (data) {
    Object.keys(data).forEach(function (prefixedKey) {
      var _a;
      var _b = parseKey(prefixedKey),
        index = _b.index,
        originalKey = _b.originalKey;
      if (!splitResults[index].data) {
        splitResults[index].data =
          ((_a = {}), (_a[originalKey] = data[prefixedKey]), _a);
      } else {
      ...
      if (error.path) {
        ...
        if (parsedKey) {
          var index = parsedKey.index,
            originalKey = parsedKey.originalKey;
          var newError = utils.relocatedError(
            error,
            tslib.__spreadArray(
              [originalKey],
              tslib.__read(error.path.slice(1))
            )
          );
...
function createBatchingExecutor(
  ...
  return function (executionParams) {
    return loader.load(executionParams);
  };
}
function createLoadFn(executor, extensionsReducer) {
  var _this = this;
  return function (execs) {
    return tslib.__awaiter(_this, void 0, void 0, function () {
      ...
      return tslib.__generator(this, function (_a) {
        ...
        operationType = graphql.getOperationAST(
          exec.document,
          undefined
        ).operation;
        while (++index < execs.length) {
          currentOperationType = graphql.getOperationAST(
        ...
        execBatches.forEach(function (execBatch) {
          var mergedExecutionParams = mergeExecutionParams(
            execBatch,
            extensionsReducer
          );
          executionResults.push(
            new valueOrPromise.ValueOrPromise(function () {
              return executor(mergedExecutionParams);
            })
          );
        });
        return [
          2 /*return*/,
          valueOrPromise.ValueOrPromise.all(executionResults)
...
var getBatchingExecutor = memoize2of4(function (
  _context,
  executor,
  dataLoaderOptions,
  extensionsReducer
