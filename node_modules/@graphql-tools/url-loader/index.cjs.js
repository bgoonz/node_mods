...
class UrlLoader {
  ...
  buildExecutor({
    ...
    const executor = ({ document, variables, ...rest }) => {
      ...
      switch (method) {
        ...
        case "POST":
          if (multipart) {
            fetchResult = this.createFormDataFromVariables({
              query,
              variables,
            }).then((form) =>
              fetch(HTTP_URL, {
      ...
      if (isPromise(fetchResult)) {
        return fetchResult.then(async (res) => {
          ...
          if (utils.isAsyncIterable(maybeStream)) {
            return utils.withCancel(
              utils.mapAsyncIterator(maybeStream, (part) => {
                if (part.json) {
                  ...
                  if (chunk.path) {
                    if (chunk.data) {
                      ...
                      lodash.merge(
                        response,
                        lodash.set({}, path.concat(chunk.path), chunk.data)
                      );
                    }
                    if (chunk.errors) {
                      response.errors = (response.errors || []).concat(
                        chunk.errors
                      );
                    }
                  } else {
    ...
    if (customFetch) {
      if (typeof customFetch === "string") {
        ...
        if (isPromise(moduleResult)) {
          return moduleResult.then((module) =>
            fetchFnName ? module[fetchFnName] : module
          );
        } else {
  ...
  async getExecutorAndSubscriberAsync(pointer, options = {}) {
    ...
    const defaultMethod = this.getDefaultMethodFromOptions(
      options.method,
      "POST"
    );
    ...
    if (options.useSSEForSubscription) {
      ...
      const connectionParams = () => ({
        headers: this.getHeadersFromOptions(options.headers, {}),
      });
  ...
  getExecutorAndSubscriberSync(pointer, options) {
    ...
    let subscriber;
    if (options.useSSEForSubscription) {
      const asyncFetchFn = (...args) =>
        this.getFetch(
          options === null || options === void 0 ? void 0 : options.customFetch,
          asyncImport,
          true
        ).then((asyncFetch) => asyncFetch(...args));
      subscriber = this.buildSSESubscriber(
        subscriptionsEndpoint,
        options.headers,
        asyncFetchFn,
  ...
  async getSubschemaConfigAsync(pointer, options) {
    const { executor, subscriber } = await this.getExecutorAndSubscriberAsync(
      pointer,
      options
    );
    return {
  ...
  getSubschemaConfigSync(pointer, options) {
    const { executor, subscriber } = this.getExecutorAndSubscriberSync(
      pointer,
      options
    );
    return {
  ...
  async load(pointer, options) {
    ...
    const subschemaConfig = await this.getSubschemaConfigAsync(
      pointer,
      options
    );
...
function switchProtocols(pointer, protocolMap) {
  const protocols = Object.keys(protocolMap).map((source) => [
    source,
    protocolMap[source],
  ]);
