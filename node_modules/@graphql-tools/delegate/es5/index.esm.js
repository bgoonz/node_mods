...
function createRequest(_a) {
  ...
  if (sourceSchema != null && variableDefinitions != null) {
    variableDefinitions.forEach(function (def) {
      ...
      var serializedValue = serializeInputValue(
        varType,
    ...
    if (source instanceof Error || source === null) {
      ...
      Object.keys(fieldNodes_1).forEach(function (responseKey) {
        if (source instanceof GraphQLError) {
          nullResult_1[responseKey] = relocatedError(
            source,
            path.concat([responseKey])
          );
        } else if (source instanceof Error) {
          nullResult_1[responseKey] = locatedError(
            source,
...
function cloneSubschemaConfig(subschemaConfig) {
  var newSubschemaConfig = __assign(__assign({}, subschemaConfig), {
    transforms:
      subschemaConfig.transforms != null
        ? __spreadArray([], __read(subschemaConfig.transforms))
        : undefined,
  });
  if (newSubschemaConfig.merge != null) {
    ...
    Object.keys(newSubschemaConfig.merge).forEach(function (typeName) {
      var mergedTypeConfig = (newSubschemaConfig.merge[typeName] = __assign(
        {},
      ...
      if (mergedTypeConfig.entryPoints != null) {
        mergedTypeConfig.entryPoints = mergedTypeConfig.entryPoints.map(
          function (entryPoint) {
            return __assign({}, entryPoint);
          }
        );
      }
      if (mergedTypeConfig.fields != null) {
        var fields_1 = (mergedTypeConfig.fields = __assign(
          {},
...
function mergeFields(
  ...
  delegationMap.forEach(function (selectionSet, s) {
    ...
    var valueOrPromise = new ValueOrPromise(function () {
  ...
  var newDefinitions = operations.map(function (operation) {
    ...
    Object.keys(fields).forEach(function (responseKey) {
      ...
      fieldNodes.forEach(function (fieldNode) {
        ...
        newSelections.push(
          __assign(__assign({}, fieldNode), { selectionSet: newSelectionSet })
        );
      });
    });
...
function expandAbstractTypes(
  ...
  var newDocument = __assign(__assign({}, document), {
    definitions: __spreadArray(
      __spreadArray([], __read(operations)),
      __read(newFragments)
    ),
  });
...
function filterSelectionSet(schema, type, validFragments, selectionSet) {
  ...
  var filteredSelectionSet = visit(
    selectionSet,
    visitWithTypeInfo(
      typeInfo,
      ((_a = {}),
...
function delegateRequest(_a) {
  ...
  if (targetOperation === "query" || targetOperation === "mutation") {
    ...
    if (
      ...
      executor_1 = getBatchingExecutor(
        context,
        executor_1,
    ...
    return new ValueOrPromise(function () {
      return executor_1(
        __assign(__assign({}, processedRequest), {
          context: context,
          info: info,
        })
      );
  ...
  return subscriber(
    __assign(__assign({}, processedRequest), { context: context, info: info })
