...
function createRequest({
  ...
  if (selectionSet != null) {
    ...
    const selections = fieldNodes.reduce(
      (acc, fieldNode) =>
    ...
    argumentNodeMap = {};
    const args =
      (_a = fieldNodes[0]) === null || _a === void 0 ? void 0 : _a.arguments;
    if (args) {
...
const getFieldsNotInSubschema = memoizeInfoAnd2Objects(function (
  info,
  subschema,
  mergedTypeInfo
) {
...
function mergeFields(
  ...
  const { proxiableSubschemas, nonProxiableSubschemas } =
    sortSubschemasByProxiability(
      mergedTypeInfo,
      sourceSubschemaOrSourceSubschemas,
      targetSubschemas,
      fieldNodes
  ...
  delegationMap.forEach((selectionSet, s) => {
    ...
    const valueOrPromise$1 = new valueOrPromise.ValueOrPromise(() =>
      resolver(object, context, info, s, selectionSet)
    ).catch((error) => error);
  ...
  for (const selection of selectionSet.selections) {
    if (selection.kind === graphql.Kind.FIELD) {
      ...
      if (selection.selectionSet != null) {
        return typesContainSelectionSet(
          fields.map((field) => graphql.getNamedType(field.type)),
  ...
  const newDefinitions = operations.map((operation) => {
    ...
    Object.keys(fields).forEach((responseKey) => {
      ...
      fieldNodes.forEach((fieldNode) => {
        ...
        const newSelectionSet = graphql.visit(
          selectionSet,
...
function wrapConcreteTypes(returnType, targetSchema, document) {
  ...
  const queryTypeName =
    (_a = targetSchema.getQueryType()) === null || _a === void 0
      ? void 0
      : _a.name;
  const mutationTypeName =
    (_b = targetSchema.getMutationType()) === null || _b === void 0
      ? void 0
      : _b.name;
  const subscriptionTypeName =
    (_c = targetSchema.getSubscriptionType()) === null || _c === void 0
      ? void 0
      : _c.name;
  const typeInfo = new graphql.TypeInfo(targetSchema);
...
function filterToSchema(targetSchema, document, variables) {
  ...
  const validFragmentsWithType = validFragments.reduce(
    (prev, fragment) => ({
      ...prev,
          ...
          if (
            ...
            if (node.arguments != null) {
              const args = node.arguments.filter(
                (arg) => argNames.indexOf(arg.name.value) !== -1
              );
...
function mergeDataAndErrors(data, errors, path, onLocatedError, index = 1) {
  if (data == null) {
    ...
    const newError = graphql.locatedError(
...
class AddArgumentsAsVariables {
  ...
  transformRequest(originalRequest, delegationContext, _transformationContext) {
    const { document, variables } = addVariablesToRootField(
      delegationContext.targetSchema,
      originalRequest,
      this.args
    );
...
class Transformer {
  constructor(context, binding = defaultDelegationBinding) {
    ...
    delegationTransforms.forEach((transform) =>
...
function delegateRequest({
  ...
  const { targetSchema, targetRootValue, subschemaConfig, allTransforms } =
    collectTargetParameters(
    ...
    returnType:
      (_a =
        returnType !== null && returnType !== void 0
          ...
          ? void 0
          : info.returnType) !== null && _a !== void 0
        ? _a
    ...
    transformedSchema:
      (_c =
        transformedSchema !== null && transformedSchema !== void 0
          ? transformedSchema
          : (_b = subschemaConfig) === null || _b === void 0
          ? void 0
          : _b.transformedSchema) !== null && _c !== void 0
        ? _c
        : targetSchema,
    skipTypeMerging,
  };
