...
function normalizePointers(unnormalizedPointerOrPointers) {
  return utils
    .asArray(unnormalizedPointerOrPointers)
    .reduce(function (normalizedPointers, unnormalizedPointer) {
    ...
    return tslib.__generator(this, function (_d) {
      switch (_d.label) {
        ...
        case 8:
          error_1 = _d.sent();
          utils.debugLog(
            "Failed to find any GraphQL type definitions in: " +
              pointer +
              " - " +
              error_1.message
          );
...
function loadFileSync(pointer, options) {
  ...
  try {
    for (
      var _b = tslib.__values(options.loaders), _c = _b.next();
      !_c.done;
      _c = _b.next()
    ) {
      var loader = _c.value;
      try {
        var canLoad =
          loader.canLoadSync &&
          loader.loadSync &&
          loader.canLoadSync(pointer, options);
        if (canLoad) {
...
function useQueue(options) {
  ...
  var queue = [];
  var limit = (
    options === null || options === void 0 ? void 0 : options.concurrency
  )
...
function collectSources(_a) {
  var _b;
  var pointerOptionMap = _a.pointerOptionMap,
    options = _a.options;
  return tslib.__awaiter(this, void 0, void 0, function () {
    var sources,
      globs,
      ...
      switch (_d.label) {
        case 0:
          ...
          (_c = createHelpers({
            ...
            stack: [
              collectDocumentString,
              collectGlob,
              collectCustomLoader,
              collectFallback,
            ],
          })),
            (addSource = _c.addSource),
            (addGlob = _c.addGlob),
            (collect = _c.collect);
          for (pointer in pointerOptionMap) {
            pointerOptions = tslib.__assign(
              tslib.__assign(
                {},
                (_b = pointerOptionMap[pointer]) !== null && _b !== void 0
                  ? _b
                  : {}
              ),
              { unixify: unixify }
          ...
          return [
            4 /*yield*/,
            globby__default(globs, createGlobbyOptions(options)),
          ];
        case 1:
          paths = _d.sent();
...
function collectSourcesSync(_a) {
  ...
  for (var pointer in pointerOptionMap) {
    var pointerOptions = tslib.__assign(
      tslib.__assign(
        {},
        (_b = pointerOptionMap[pointer]) !== null && _b !== void 0 ? _b : {}
      ),
      { unixify: unixify }
    );
    collect({
...
function createHelpers(_a) {
  ...
  var collect = useStack.apply(
    void 0,
    tslib.__spreadArray([], tslib.__read(stack))
  );
  var addGlob = function (_a) {
    var pointerOptions = _a.pointerOptions,
      pointer = _a.pointer;
    globs.push(pointer);
...
function includeIgnored(_a) {
  var options = _a.options,
    globs = _a.globs;
  if (options.ignore) {
    var ignoreList = utils
      .asArray(options.ignore)
      .map(function (g) {
        return "!(" + g + ")";
      })
      .map(unixify);
    if (ignoreList.length > 0) {
      globs.push.apply(
        globs,
        tslib.__spreadArray([], tslib.__read(ignoreList))
      );
    }
  }
}
function createGlobbyOptions(options) {
  return tslib.__assign(tslib.__assign({ absolute: true }, options), {
    ignore: [],
  });
}
function collectSourcesFromGlobals(_a) {
  var filepaths = _a.filepaths,
    options = _a.options,
    globOptions = _a.globOptions,
    pointerOptionMap = _a.pointerOptionMap,
...
function collectDocumentString(_a, next) {
  var pointer = _a.pointer,
    pointerOptions = _a.pointerOptions,
    options = _a.options,
    addSource = _a.addSource,
    queue = _a.queue;
  if (utils.isDocumentString(pointer)) {
    return queue(function () {
      var source = utils.parseGraphQLSDL(
        stringToHash(pointer) + ".graphql",
        pointer,
        tslib.__assign(tslib.__assign({}, options), pointerOptions)
      );
      addSource({
        source: source,
        pointer: pointer,
        ...
        return tslib.__generator(this, function (_a) {
          switch (_a.label) {
            case 0:
              return [
                4 /*yield*/,
                useCustomLoader(pointerOptions.loader, options.cwd),
              ];
            case 1:
              loader = _a.sent();
              return [
...
var filterKind = function (content, filterKinds) {
  ...
  if (
    ...
    try {
      for (
        var _b = tslib.__values(content.definitions), _c = _b.next();
        !_c.done;
        _c = _b.next()
      ) {
    ...
    if (invalidDefinitions.length > 0) {
      invalidDefinitions.forEach(function (d) {
        utils.debugLog(
          "Filtered document of kind " +
            d.kind +
            " due to filter policy (" +
            filterKinds.join(", ") +
            ")"
        );
      });
...
function parseSource(_a) {
  var partialSource = _a.partialSource,
    options = _a.options,
    globOptions = _a.globOptions,
    pointerOptionMap = _a.pointerOptionMap,
    addValidSource = _a.addValidSource;
  if (partialSource) {
...
function prepareInput(_a) {
  var source = _a.source,
    ...
    globOptions = _a.globOptions,
    pointerOptionMap = _a.pointerOptionMap;
  var specificOptions = tslib.__assign(
    tslib.__assign({}, options),
    source.location in pointerOptionMap
      ? globOptions
...
function parseSchema(input) {
  if (input.source.schema) {
    input.source.schema = utils.fixSchemaAst(
      input.source.schema,
      input.options
    );
    input.source.rawSDL = utils.printSchemaWithDirectives(
      input.source.schema,
      input.options
    );
  }
...
function parseRawSDL(input) {
  if (input.source.rawSDL) {
    input.source.document = utils.parseGraphQLSDL(
...
function useKindsFilter(input) {
  if (input.options.filterKinds) {
    input.source.document = filterKind(
      input.source.document,
      input.options.filterKinds
    );
  }
}
...
function collectValidSources(input, addValidSource) {
  if (
    ...
    input.source.document.definitions.length > 0
  ) {
      ...
      switch (_a.label) {
        ...
        case 1:
          ...
          return [
            4 /*yield*/,
            Promise.all(
              sources.map(function (partialSource) {
...
function prepareResult(_a) {
  var options = _a.options,
    pointerOptionMap = _a.pointerOptionMap,
    validSources = _a.validSources;
  var pointerList = Object.keys(pointerOptionMap);
  if (pointerList.length > 0 && validSources.length === 0) {
    throw new Error(
      "\n      Unable to find any GraphQL type definitions for the following pointers:\n        " +
        pointerList.map(function (p) {
          return "\n          - " + p + "\n          ";
        })
    );
  }
  return options.sort
    ? validSources.sort(function (left, right) {
        return utils.compareStrings(left.location, right.location);
      })
    : validSources;
}
...
var NON_OPERATION_KINDS = Object.keys(graphql.Kind)
  .reduce(function (prev, v) {
    return tslib.__spreadArray(tslib.__spreadArray([], tslib.__read(prev)), [
      graphql.Kind[v],
    ]);
  }, [])
  .filter(function (v) {
    return !OPERATION_KINDS.includes(v);
  });
/**
...
function loadDocuments(pointerOrPointers, options) {
  return loadTypedefs(
    pointerOrPointers,
    tslib.__assign(
      { noRequire: true, filterKinds: NON_OPERATION_KINDS },
      options
    )
  );
}
      ...
      switch (_b.label) {
        case 0:
          return [
            ...
            loadTypedefs(
              schemaPointers,
              tslib.__assign({ filterKinds: OPERATION_KINDS }, options)
            ),
          ];
        case 1:
          sources = _b.sent();
          (_a = collectSchemasAndTypeDefs(sources)),
            (schemas = _a.schemas),
            (typeDefs = _a.typeDefs);
          mergeSchemasOptions = tslib.__assign(
            { schemas: schemas, typeDefs: typeDefs },
            options
          );
...
function loadSchemaSync(schemaPointers, options) {
  var sources = loadTypedefsSync(
    schemaPointers,
    tslib.__assign({ filterKinds: OPERATION_KINDS }, options)
  );
  var _a = collectSchemasAndTypeDefs(sources),
    schemas = _a.schemas,
    typeDefs = _a.typeDefs;
  var mergeSchemasOptions = tslib.__assign(
    { schemas: schemas, typeDefs: typeDefs },
    options
  );
  var schema = merge.mergeSchemas(mergeSchemasOptions);
  if (
    options === null || options === void 0 ? void 0 : options.includeSources
  ) {
    includeSources(schema, sources);
  }
  return schema;
...
function includeSources(schema, sources) {
  schema.extensions = tslib.__assign(tslib.__assign({}, schema.extensions), {
    sources: sources
      .filter(function (source) {
        return source.rawSDL || source.document;
      })
      .map(function (source) {
        return new graphql.Source(
          source.rawSDL || graphql.print(source.document),
          source.location
        );
      }),
  });
