...
function normalizePointers(unnormalizedPointerOrPointers) {
  return utils
    ...
    .reduce((normalizedPointers, unnormalizedPointer) => {
...
async function loadFile(pointer, options) {
  ...
  for await (const loader of options.loaders) {
    try {
      ...
      utils.debugLog(
        `Failed to find any GraphQL type definitions in: ${pointer} - ${error.message}`
      );
      throw error;
    }
  }
...
function collectSourcesFromGlobalsSync({
  ...
  const collectFromGlobs = useStack(
    collectCustomLoaderSync,
    collectFallbackSync
  );
  for (let i = 0; i < filepaths.length; i++) {
...
function collectDocumentString(
  { pointer, pointerOptions, options, addSource, queue },
  next
) {
  if (utils.isDocumentString(pointer)) {
    return queue(() => {
      const source = utils.parseGraphQLSDL(
      ...
      invalidDefinitions.forEach((d) => {
        utils.debugLog(
          `Filtered document of kind ${
            d.kind
          } due to filter policy (${filterKinds.join(", ")})`
        );
      });
    }
    return {
...
function prepareResult({ options, pointerOptionMap, validSources }) {
  ...
  return options.sort
    ? validSources.sort((left, right) =>
        utils.compareStrings(left.location, right.location)
      )
    : validSources;
}
