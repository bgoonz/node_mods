...
import {
  ...
  mergeSchemasAsync,
  mergeSchemas,
} from "@graphql-tools/merge/es5";

function normalizePointers(unnormalizedPointerOrPointers) {
  return asArray(unnormalizedPointerOrPointers).reduce(function (
    normalizedPointers,
    unnormalizedPointer
  ) {
...
function loadFileSync(pointer, options) {
  ...
  try {
    for (
      var _b = __values(options.loaders), _c = _b.next();
      !_c.done;
      _c = _b.next()
    ) {
      var loader = _c.value;
...
function collectDocumentString(_a, next) {
  ...
  if (isDocumentString(pointer)) {
    return queue(function () {
      var source = parseGraphQLSDL(
        stringToHash(pointer) + ".graphql",
        pointer,
        __assign(__assign({}, options), pointerOptions)
      );
      addSource({
        source: source,
        pointer: pointer,
...
var filterKind = function (content, filterKinds) {
  ...
  if (
    ...
    try {
      for (
        var _b = __values(content.definitions), _c = _b.next();
        !_c.done;
        _c = _b.next()
      ) {
...
function parseSchema(input) {
  if (input.source.schema) {
    ...
    input.source.rawSDL = printSchemaWithDirectives(
      input.source.schema,
      input.options
    );
  }
...
function prepareResult(_a) {
  ...
  return options.sort
    ? validSources.sort(function (left, right) {
        return compareStrings(left.location, right.location);
      })
    : validSources;
}
...
var NON_OPERATION_KINDS = Object.keys(Kind)
  .reduce(function (prev, v) {
    return __spreadArray(__spreadArray([], __read(prev)), [Kind[v]]);
  }, [])
  .filter(function (v) {
...
function loadDocuments(pointerOrPointers, options) {
  return loadTypedefs(
    pointerOrPointers,
    __assign({ noRequire: true, filterKinds: NON_OPERATION_KINDS }, options)
  );
}
/**
      ...
      switch (_b.label) {
        case 0:
          return [
            ...
            loadTypedefs(
              schemaPointers,
              __assign({ filterKinds: OPERATION_KINDS }, options)
            ),
          ];
        case 1:
          sources = _b.sent();
...
function loadSchemaSync(schemaPointers, options) {
  ...
  var _a = collectSchemasAndTypeDefs(sources),
    schemas = _a.schemas,
    typeDefs = _a.typeDefs;
  var mergeSchemasOptions = __assign(
    { schemas: schemas, typeDefs: typeDefs },
    options
  );
...
function includeSources(schema, sources) {
  schema.extensions = __assign(__assign({}, schema.extensions), {
    sources: sources
      ...
      .map(function (source) {
        return new Source(
          source.rawSDL || print(source.document),
          source.location
        );
      }),
  });
