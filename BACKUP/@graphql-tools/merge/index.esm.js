    ...
    leave: {
      ...
      DirectiveDefinition: addDescription(
        ({ name, arguments: args, locations }) =>
          `directive @${name}${wrap("(", join(args, ", "), ")")} on ${join(
            locations,
            " | "
          )}`
      ),
      ...
      if (existingArg.value.kind === "ListValue") {
        ...
        existingArg.value.values = deduplicateLists(
          ...
          (targetVal, source) => {
            ...
            return (
              !value || !source.some((sourceVal) => sourceVal.value === value)
            );
          }
        );
      } else {
...
function mergeFields(type, f1, f2, config) {
  ...
  if (config && config.exclusions) {
    return result.filter(
      (field) =>
        !config.exclusions.includes(`${type.name.value}.${field.name.value}`)
    );
  }
  return result;
}
...
function mergeGraphQLNodes(nodes, config) {
  ...
  for (const nodeDefinition of nodes) {
    if (isNamedDefinitionNode(nodeDefinition)) {
      ...
      if (
        ((_a =
          config === null || config === void 0 ? void 0 : config.exclusions) ===
          null || _a === void 0
          ? void 0
          : _a.includes(name + ".*")) ||
        ((_b =
...
function mergeExtensions(extensions) {
  return extensions.reduce(
    (result, extensionObj) => [result, extensionObj].reduce(mergeDeep, {}),
    {}
  );
}
...
function extractExtensionsFromSchema(schema) {
  ...
  travelSchemaPossibleExtensions(schema, {
    ...
    onEnumValue: (type, value) =>
      (result.types[type.name].values[value.name] = value.extensions || {}),
    onScalar: (type) =>
      (result.types[type.name] = {
        type: "scalar",
        extensions: type.extensions || {},
...
async function mergeSchemasAsync(config) {
  const [typeDefs, resolvers, extensions] = await Promise.all([
    mergeTypes(config),
    Promise.all(
      config.schemas.map(async (schema) => getResolversFromSchema(schema))
    ...
    Promise.all(
      config.schemas.map(async (schema) => extractExtensionsFromSchema(schema))
...
function mergeTypes({ schemas, typeDefs, ...config }) {
  return mergeTypeDefs(
    [...schemas, ...(typeDefs ? asArray(typeDefs) : [])],
    config
  );
}
