...
const flattenArray = (arr) =>
  arr.reduce(
    (acc, next) => acc.concat(Array.isArray(next) ? flattenArray(next) : next),
    []
  );
    ...
    Object.entries(config.fields).forEach(([fieldName, field]) => {
      if (
        ...
        for (const argName of Object.keys(field.args)) {
          if (
            !argumentFilter(operation, fieldName, argName, field.args[argName])
          ) {
  ...
  if (fieldFilter || argumentFilter) {
    ...
    Object.entries(config.fields).forEach(([fieldName, field]) => {
      if (
        ...
        for (const argName of Object.keys(field.args)) {
          if (
            !argumentFilter(type.name, fieldName, argName, field.args[argName])
          ) {
            delete field.args[argName];
          }
        }
...
function addTypes(schema, newTypesOrDirectives) {
  ...
  return new GraphQLSchema({
    ...
    subscription:
      subscriptionTypeName != null ? typeMap[subscriptionTypeName] : undefined,
    types: Object.keys(typeMap).map((typeName) => typeMap[typeName]),
    directives,
  });
}
