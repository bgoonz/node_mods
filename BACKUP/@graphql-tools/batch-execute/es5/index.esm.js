    ...
    prefixedExecutionParams.document.definitions.forEach(function (def) {
      ...
      if (isOperationDefinition(def)) {
        ...
        mergedSelections.push.apply(
          mergedSelections,
          __spreadArray([], __read(def.selectionSet.selections))
        );
  ...
  return {
    document: {
      ...
      definitions: __spreadArray(
        ...
        __read(mergedFragmentDefinitions)
      ),
    },
...
function aliasTopLevelFields(prefix, document) {
  ...
  var transformer =
    ...
    (_a[Kind.OPERATION_DEFINITION] = function (def) {
      ...
      return __assign(__assign({}, def), {
        selectionSet: __assign(__assign({}, def.selectionSet), {
  ...
  return visit(
    document,
    transformer,
    ((_b = {}), (_b[Kind.DOCUMENT] = ["definitions"]), _b)
  );
}
/**
...
function aliasFieldsInInlineFragment(prefix, fragment, document) {
  ...
  return __assign(__assign({}, fragment), {
    selectionSet: __assign(__assign({}, fragment.selectionSet), {
      selections: aliasFieldsInSelection(prefix, selections, document),
    }),
  });
}
...
function prefixNodeName(namedNode, prefix) {
  return __assign(__assign({}, namedNode), {
    name: __assign(__assign({}, namedNode.name), {
...
function aliasField(field, aliasPrefix) {
  ...
  return __assign(__assign({}, field), {
    alias: __assign(__assign({}, aliasNode), {
    ...
    errors.forEach(function (error) {
      if (error.path) {
        ...
        if (parsedKey) {
          ...
          var newError = relocatedError(
            error,
            __spreadArray([originalKey], __read(error.path.slice(1)))
          );
    ...
    return __awaiter(_this, void 0, void 0, function () {
      ...
      return __generator(this, function (_a) {
        ...
        while (++index < execs.length) {
          currentOperationType = getOperationAST(
        ...
        return [
          2 /*return*/,
          ValueOrPromise.all(executionResults)
