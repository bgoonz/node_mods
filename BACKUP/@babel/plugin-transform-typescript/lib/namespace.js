...
function handleVariableDeclaration(node, name, hub) {
  ...
  if (
    declarations.every((declarator) => _core.types.isIdentifier(declarator.id))
  ) {
    for (const declarator of declarations) {
      declarator.init = _core.types.assignmentExpression(
        "=",
        getMemberExpression(_core.types, name, declarator.id.name),
        declarator.init
      );
    }
  ...
  for (const idName in bindingIdentifiers) {
    assignments.push(
      _core.types.assignmentExpression(
        "=",
        getMemberExpression(_core.types, name, idName),
  ...
  return [
    ...
    _core.types.expressionStatement(
      _core.types.sequenceExpression(assignments)
    ),
  ];
}
