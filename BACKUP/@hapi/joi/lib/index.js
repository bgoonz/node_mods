...
internals.root = function () {
  ...
  root.reach = function (schema, path) {
    ...
    Hoek.assert(
      Array.isArray(path) || typeof path === "string",
    ...
    const schemaPath =
      typeof path === "string" ? (path ? path.split(".") : []) : path.slice();
  ...
  root.defaults = function (fn) {
    ...
    Hoek.assert(
      joi && joi instanceof this.constructor,
    ...
    joi._defaults = (schema) => {
      if (this._defaults) {
        schema = this._defaults(schema);
        Hoek.assert(
          schema instanceof this.constructor,
          "defaults() must return a schema"
        );
      }
  ...
  root.extend = function (...args) {
    ...
    for (let i = 0; i < extensions.length; ++i) {
      ...
      if (extension.language) {
        ...
        type.prototype._language = Hoek.applyToDefaults(
          type.prototype._language ||
            (base._settings && base._settings.language) ||
            {},
          lang
        );
      }
      ...
      if (extension.coerce) {
        type.prototype._coerce = function (value, state, options) {
          if (ctor.prototype._coerce) {
            const baseRet = ctor.prototype._coerce.call(
              this,
              value,
              state,
              options
      ...
      if (extension.pre) {
        type.prototype._base = function (value, state, options) {
          if (ctor.prototype._base) {
            const baseRet = ctor.prototype._base.call(
              this,
              value,
              state,
              options
        ...
        for (let j = 0; j < extension.rules.length; ++j) {
          ...
          const ruleArgs = rule.params
            ? rule.params instanceof Any
              ? rule.params._inner.children.map((k) => k.key)
              : Object.keys(rule.params)
            : [];
          const validateArgs = rule.params
            ? Cast.schema(this, rule.params)
            : null;
            ...
            if (rule.setup) {
              ...
              if (newSchema !== undefined) {
                Hoek.assert(
                  ...
                  `Setup of extension Joi.${this._type}().${rule.name}() must return undefined or a Joi object`
                );
                schema = newSchema;
              }
  ...
  root.extensionsSchema = internals.array
    .items([internals.object, internals.func.arity(1)])
    .strict();
