...
internals.Object = class extends Any {
  ...
  _base(value, state, options) {
    ...
    for (let i = 0; i < this._inner.renames.length; ++i) {
      ...
      if (rename.isRegExp) {
        ...
        const allUndefined = matchedTargetKeys.every(
        ...
        if (
          Object.prototype.hasOwnProperty.call(target, rename.to) &&
        ...
        if (allUndefined) {
          ...
          target[rename.to] =
            target[matchedTargetKeys[matchedTargetKeys.length - 1]];
        }
      ...
      for (let i = 0; i < this._inner.children.length; ++i) {
        ...
        if (result.errors) {
          ...
          if (
            ...
            (result.value === undefined && result.value !== item)
          ) {
            stripProps.push(key);
    ...
    if (
      ...
      if (
        (options.stripUnknown && this._flags.allowUnknown !== true) ||
      ...
      if (
        ...
        for (const unprocessedKey of unprocessed) {
          errors.push(
            this.createError(
              "object.allowUnknown",
              { child: unprocessedKey, value: target[unprocessedKey] },
              {
                key: unprocessedKey,
    ...
    for (let i = 0; i < this._inner.dependencies.length; ++i) {
      ...
      const localState = hasKey
        ? new State(splitKey[splitKey.length - 1], [...state.path, ...splitKey])
        : new State(null, state.path);
      const err = internals[dep.type].call(
        this,
        dep.key,
        hasKey && Hoek.reach(target, dep.key, { functions: true }),
        dep.peers,
        target,
        localState,
        options
  ...
  pattern(pattern, schema) {
    ...
    Hoek.assert(
      isRegExp || pattern instanceof Any,
  ...
  rename(from, to, options) {
    Hoek.assert(
      typeof from === "string" || from instanceof RegExp,
