...
module.exports = internals.Any = class {
  ...
  concat(schema) {
    ...
    obj._settings = obj._settings
      ? Settings.concat(obj._settings, schema._settings)
      : schema._settings;
  ...
  applyFunctionToChildren(children, fn, args = [], root) {
    ...
    if (children.length !== 1 || children[0] !== "") {
      ...
      const extraChildren = (
        children[0] === "" ? children.slice(1) : children
      ).map((child) => {
        return root + child;
      });
  ...
  when(condition, options) {
    ...
    const then = options.hasOwnProperty("then")
      ? this.concat(Cast.schema(this._currentJoi, options.then))
      : undefined;
    const otherwise = options.hasOwnProperty("otherwise")
  ...
  notes(notes) {
    Hoek.assert(
      notes && (typeof notes === "string" || Array.isArray(notes)),
  ...
  tags(tags) {
    Hoek.assert(
      tags && (typeof tags === "string" || Array.isArray(tags)),
    ...
    for (let i = 0; i < examples.length; ++i) {
      ...
      if (options !== undefined) {
        Hoek.assert(
          ...
          `Options for example at index ${i} must be an object`
        );
        const unknownOptions = Object.keys(options).filter(
          (option) => !["parent", "context"].includes(option)
        );
        Hoek.assert(
          unknownOptions.length === 0,
          `Unknown example options ${unknownOptions} at index ${i}`
        );
      } else {
        options = {};
      }
  ...
  _validate(value, state, options, reference) {
    ...
    if (this._settings) {
      ...
      if (isDefaultOptions && this._settings[Symbols.settingsCache]) {
        ...
        options = Settings.concat(
          this._language
            ? Settings.concat({ language: this._language }, options)
            : options,
          this._settings
        );
        if (isDefaultOptions) {
    ...
    if (this._coerce) {
      ...
      if (coerced.errors) {
        ...
        return this._finalizeValue(
          value,
          originalValue,
          errors,
          state,
          options
        ); // Coerced error always aborts early
      }
    ...
    if (this._invalids.has(value, state, options, this._flags.insensitive)) {
      errors.push(
        this.createError(
          ...
          { value, invalids: this._invalids.values({ stripUndefined: true }) },
          state,
          options
        )
      );
    ...
    if (this._base) {
      ...
      if (base.errors) {
        ...
        return this._finalizeValue(
          ...
          state,
          options
        ); // Base error always aborts early
      }
      ...
      if (base.value !== value) {
        ...
        if (
          this._invalids.has(value, state, options, this._flags.insensitive)
        ) {
          errors.push(
            this.createError(
              value === "" ? "any.empty" : "any.invalid",
              {
                value,
                invalids: this._invalids.values({ stripUndefined: true }),
              },
              state,
              options
            )
    ...
    if (this._flags.allowOnly) {
      errors.push(
        this.createError(
          "any.allowOnly",
          { value, valids: this._valids.values({ stripUndefined: true }) },
          state,
          options
        )
      );
  ...
  _finalizeValue(value, originalValue, errors, state, options) {
    ...
    if (value !== undefined) {
      ...
      typeof this._flags.default === "function" &&
  ...
  describe() {
    ...
    if (flags.length) {
      if (
        ["empty", "default", "lazy", "label"].some((flag) =>
          this._flags.hasOwnProperty(flag)
        )
      ) {
        description.flags = {};
