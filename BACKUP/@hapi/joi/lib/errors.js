...
exports.Err = class {
  constructor(type, context, state, options, flags, message, template) {
    ...
    if (this.flags.label) {
      ...
      localized && // language can be null for arrays exclusion check
  ...
  toString() {
    ...
    if (format === null) {
      const childrenString = internals.stringify(
        this.context.reason,
        wrapArrays
      );
      if (wrapArrays) {
    ...
    const message = format.replace(/{{(!?)([^}]+)}}/g, ($0, isSecure, name) => {
      ...
      return isSecure && this.options.escapeHtml
  ...
  const processErrors = function (localErrors, parent, overrideMessage) {
    for (let i = 0; i < localErrors.length; ++i) {
      ...
      if (item.context.reason) {
        const override = processErrors(
          item.context.reason,
          item.path,
          item.type === "override" ? item.message : null
        );
        if (override) {
          return override;
        }
...
internals.annotate = function (stripColorCodes) {
  ...
  for (let i = this.details.length - 1; i >= 0; --i) {
    ...
    for (let j = 0; ; ++j) {
      ...
      if (j + 1 < path.length && ref[seg] && typeof ref[seg] !== "string") {
        ...
        const refAnnotations = (ref[internals.annotations] = ref[
  ...
  let message = internals
    .safeStringify(obj, 2)
    .replace(replacers.key, ($0, $1) => `" ${redFgEscape}[${$1}]${endColor}`)
    .replace(
      replacers.missing,
      ($0, $1, $2) =>
        `${redBgEscape}"${$1}"${endColor}${redFgEscape} [${$2}]: -- missing --${endColor}`
    )
    .replace(
