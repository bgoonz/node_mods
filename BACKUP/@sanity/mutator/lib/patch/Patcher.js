  ...
  leads.forEach((lead) => {
    if (lead.target.isIndexReference()) {
      lead.target.toIndicies().forEach((i) => {
        result = result.setIndexAccessor(
          i,
          process(lead.matcher, result.getIndex(i))
        );
      });
      ...
      var newValueAccessor = process(
        lead.matcher,
        result.getAttribute(lead.target.name())
      );

      if (oldValueAccessor !== newValueAccessor) {
        result = result.setAttributeAccessor(
          lead.target.name(),
          newValueAccessor
        );
      }
      ...
      throw new Error(
        "Unable to handle target ".concat(lead.target.toString())
      );
    }
