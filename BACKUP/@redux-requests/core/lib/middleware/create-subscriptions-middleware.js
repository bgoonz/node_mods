  ...
  return function (store) {
    return function (next) {
      return function (action) {
        ...
        if (action.type === _constants.OPEN_WEBSOCKET) {
          ...
          ws.addEventListener("open", function () {
            ...
            if (isHeartbeatMessage) {
              ...
              lastHeartbeatTimeout = setTimeout(function () {
                store.dispatch((0, _actions.closeWebsocket)(3000));
        ...
        if (action.type === _constants.GET_WEBSOCKET) {
          return ws;
        }

        if (ws && action.type === _constants.CLOSE_WEBSOCKET) {
          clearLastReconnectTimeout();
          clearLastHeartbeatTimeout();
          ...
          var _requestsStore = (0, _createRequestsStore["default"])(store);
