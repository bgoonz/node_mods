...
exports.Parser = class {
  constructor(string, options = {}) {
    ...
    this.settings = options[internals.settings]
      ? options
      : Object.assign(
          { [internals.settings]: true, constants: {}, functions: {} },
          options
        );
    this.single = null;
  ...
  _parse(string) {
    ...
    this._parts = parts.map((part) => {
      ...
      if (part.type === "operator") {
        return internals.operatorsPrefix.includes(part.value)
          ? part
          : part.value;
      }
  ...
  _subFormula(string, name) {
    ...
    if (string) {
      ...
      const flush = () => {
        if (!current) {
          throw new Error(
            `Formula contains function ${name} with invalid arguments ${string}`
          );
        }
