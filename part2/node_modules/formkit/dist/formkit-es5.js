...
class Field {
    constructor(pathToField, params, form) {
        this.handleChange = (rawValue) => {
            if (this.disabled)
                return;
            const newValue = parseValue(rawValue);
            const prevValue = this.value;
            const isChanged = !isEqual(prevValue, newValue);
            if (isChanged) {
                this.updateState(() => {
            ...
            if (!this.form.config.allowSaveUnmodifiedField && !isChanged)
                return;
    ...
    $setSavedValue(newSavedValue) {
        ...
        if (this.form.config.allowFocusedFieldUpdating
    ...
    initState(rawFieldSchema) {
        const initialState = this.generateInitialState(rawFieldSchema);
        this.fieldStorage.initState(this.pathToField, initialState);
    }
    generateInitialState({ initial, disabled, defaultValue, savedValue }) {
    ...
    riseUserChangeEvent(pathToField, prevValue, newValue) {
        const eventData = {
            field: pathToField,
            prevValue,
            value: newValue,
    ...
    updateStateAndValidate(cbWhichChangesState) {
        this.updateState(() => {
            if (cbWhichChangesState)
                cbWhichChangesState();
            this.form.validate();
        });
    }
    updateState(cbWhichChangesState) {
...
function findFieldRecursively(fields, cb) {
    const recursive = (obj, rootPath) => {
        let foundField;
        find(obj, (item, name) => {
            const itemPath = trim(`${rootPath}.${name}`, FIELD_PATH_SEPARATOR);
            if (item instanceof Field) {
                const returnedValue = cb(item, itemPath);
                if (returnedValue) {
                    foundField = item;
                    return true;
                }
                ...
                foundField = recursive(item, itemPath);
                return;
            }
            else {
...
function eachFieldRecursively(fields, cb) {
    const recursive = (obj, rootPath) => {
...
function eachFieldSchemaRecursively(rootObject, cb) {
    eachRecursively(rootObject, (item, path) => {
...
function isFieldSchema(comingSchema) {
    let isSchema = false;
    const filedParams = [
        'initial',
        'disabled',
...
function eachRecursively(rootObject, cb) {
    const recursive = (obj, rootPath) => {
        each(obj, (item, name) => {
            const itemPath = trim(`${rootPath}.${name}`, FIELD_PATH_SEPARATOR);
...
function calculateDirty(editedValue, savedValue) {
...
class Storage {
    constructor() {
        ...
        this.store = {
            formState: Map(this._generateNewFormState()),
            fieldsState: {},
            values: Map(),
        };
    }
    getWholeStorageState() {
        ...
        this.eachField((field, path) => {
            set$1(store.fieldsState, path, field.toJS());
        });
        return store;
    }
    getWholeFormState() {
    ...
    eachField(cb) {
        eachRecursively(this.store.fieldsState, (field, path) => {
            if (!field || !Map.isMap(field))
                return;
            cb(field, path);
            return false;
        });
    ...
    getWholeFieldState(pathToField) {
        const fieldState = get$1(this.store.fieldsState, pathToField);
        if (!fieldState)
            return;
    ...
    getFieldState(pathToField, stateName) {
        ...
        if (!fieldState)
            return;
        return get$1(fieldState.toJS(), stateName);
    }
    getCombinedValue(pathToField) {
        const values = this.store.values.toJS();
        return get$1(values, pathToField);
    }
    setFieldState(pathToField, partlyState) {
        ...
        set$1(this.store.fieldsState, pathToField, newState);
...
class FormStorage {
    ...
    getEditedValues() {
        const editedValues = {};
        this.storage.eachField((field, path) => {
            const editedValue = field.get('editedValue');
            if (typeof editedValue === 'undefined')
                return;
            set$2(editedValues, path, editedValue);
        });
        return editedValues;
    }
    getSavedValues() {
        const savedValues = {};
        this.storage.eachField((field, path) => {
            set$2(savedValues, path, field.get('savedValue'));
        });
        return savedValues;
    }
    getUnSavedValues() {
        ...
        this.storage.eachField((field, path) => {
            const editedValue = field.get('editedValue');
            if (typeof editedValue === 'undefined' || field.get('savedValue') === editedValue)
                return;
            set$2(unSavedValues, path, field.get('editedValue'));
        });
    ...
    emitStorageEvent(newState, prevState, force) {
        if (!force && isEqual$1(prevState, newState))
            return;
        const data = {
            target: 'form',
            event: 'storage',
...
class FieldStorage {
    ...
    emitStorageEvent(pathToField, newState, prevState) {
        if (isEqual$2(prevState, newState))
            return;
    ...
    removeListener(pathToField, eventName, handlerIndex) {
...
class SubmitControl {
    ...
    canSubmit() {
        ...
        if (this.form.submitting)
            return `The form is submitting now.`;
        if (!this.form.config.allowSubmitUnchangedForm) {
            if (!this.form.dirty)
                return `The form hasn't changed.`;
        }
    }
    async startSubmit() {
        if (!this.handler)
    ...
    async runSubmitHandler(handler, values, editedValues) {
...
class SaveControl {
    ...
    canSave() {
        ...
        if (!this.form.touched)
            return `The form hasn't been modified`;
    }
    async startSaving(isImmediately) {
        if (!this.handler)
            return;
        const valuesBeforeSave = this.form.values;
        const promise = this.debouncedSave.exec(this.doSave, isImmediately);
...
const get$2 = require('lodash/get');
const set$3 = require('lodash/set');
class ValidateControl {
    ...
    validate() {
        ...
        eachFieldRecursively(this.form.fields, (field, path) => {
            const split = path.split('.');
            const minPathItems = 2;
            if (split.length < minPathItems)
                return;
            split.pop();
            const basePath = split.join();
            set$3(errors, basePath, {});
        });
        this.handler(errors, values);
        eachFieldRecursively(this.form.fields, (field, path) => {
            const invalidMsg = get$2(errors, path);
            if (isFormValid)
                isFormValid = !invalidMsg;
            const fieldPartlyState = {
                invalidMsg
            };
...
const isPlainObject$1 = require('lodash/isPlainObject');
const get$3 = require('lodash/get');
const set$4 = require('lodash/set');
class Form {
    constructor(config) {
        this.fields = {};
        ...
        this.clear = () => {
            this.updateStateAndValidate(() => {
                eachFieldRecursively(this.fields, (field) => field.$clearSilent());
            });
        };
        this.revert = () => {
            this.updateStateAndValidate(() => {
                eachFieldRecursively(this.fields, (field) => field.$revertSilent());
            });
        };
        this.reset = () => {
            this.updateStateAndValidate(() => {
                eachFieldRecursively(this.fields, (field) => field.$resetSilent());
            });
        };
        this.config = config;
    ...
    get dirty() {
        const field = findFieldRecursively(this.fields, (field) => {
            return field.dirty;
        });
    ...
    init(initialFields, validateCb) {
        ...
        if (Array.isArray(initialFields)) {
            ...
            eachFieldSchemaRecursively(initialFields, (fieldSchema, path) => {
    ...
    destroy() {
        this.flushSaving();
        const doDestroy = () => {
            eachFieldRecursively(this.fields, (field) => {
    ...
    $moveValuesToSaveLayer(values, force) {
        this.updateStateAndValidate(() => {
            eachFieldRecursively(this.fields, (field, pathToField) => {
                const savedValue = get$3(values, pathToField);
    ...
    $riseActionEvent(eventName, error) {
        const eventData = {
            error
        };
        this.$emit(eventName, eventData);
    }
    initField(pathToField, fieldParams) {
        const existentField = get$3(this.fields, pathToField);
        if (existentField) {
            throw new Error(`The field "${pathToField}" is exist! You can't reinitialize it!`);
        }
        const newField = new Field(pathToField, fieldParams, this);
        set$4(this.fields, pathToField, newField);
    }
    updateStateAndValidate(cbWhichChangesState, force) {
        this.updateState(() => {
            if (cbWhichChangesState)
                cbWhichChangesState();
            this.validate();
    ...
    updateState(cbWhichChangesState, force) {
        const prevState = this.formStorage.getWholeState();
        if (cbWhichChangesState)
            cbWhichChangesState();
        const newState = this.formStorage.getWholeState();
        this.formStorage.emitStorageEvent(newState, prevState, force);
    }
    eachRawField(values, cb) {
        eachRecursively(values, (value, path) => {
            const field = get$3(this.fields, path);
...
function newForm(config) {
    const newConfig = defaultsDeep({}, config, globalConfig);
...
export { Field, Form, newForm, setDefaultConfig, use };
//# sourceMappingURL=formkit-es5.js.map
